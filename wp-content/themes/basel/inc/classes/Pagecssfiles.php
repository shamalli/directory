...
class BASEL_Pagecssfiles {
	...
	public function hooks() {
		...
		add_action( 'woocommerce_single_product_summary', 'basel_page_css_files_disable', 59 );
		add_action( 'woocommerce_single_product_summary', 'basel_page_css_files_enable', 61 );
	}
	...
	public function enqueue_style( $key, $ignore_combined = false ) {
		$config  = basel_get_config( 'css-files' );
		$version = basel_get_theme_info( 'Version' );
		$styles_not_use = basel_get_opt( 'styles_not_use' );
	...
	public function enqueue_inline_style( $key ) {
		...
		if ( ! isset( $config[ $key ] ) || in_array( $key, $page_files, true ) || basel_get_opt( 'combined_css', false ) || isset(
				$GLOBALS['basel_page_css_ignore'] ) ) {
			return;
		}
