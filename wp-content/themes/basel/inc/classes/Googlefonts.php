...
class Google_Fonts extends Singleton {
	...
	public static function add_google_font( $font ) {
		...
		if ( basel_get_opt( 'load_all_font_weights', '1' ) ) {
			...
			foreach ( self::$_google_fonts as $font ) {
				if ( array_search( $font_to_add['font-family'], $font ) ) { // phpcs:ignore
					return;
				}
			}
		} else {
	...
	public function enqueue_fonts() {
		...
		if ( 'disable' !== basel_get_opt( 'google_font_display' ) ) {
			$link .= '&display=' . basel_get_opt( 'google_font_display' );
		}

		wp_enqueue_style( 'xts-google-fonts', '//fonts.googleapis.com/css?family=' . str_replace( '|', '%7C', $link ), array(), basel_get_theme_info( 'Version' ) );
	}
