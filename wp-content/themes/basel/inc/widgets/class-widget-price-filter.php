...
if( ! class_exists( 'BASEL_Widget_Price_Filter' ) ) {
	class BASEL_Widget_Price_Filter extends WPH_Widget {
		...
		function __construct() {
			...
			$args = array( 
				...
				'label' => esc_html__( 'BASEL Price filter', 'basel' ),
				// Widget Backend Description								
				'description' =>esc_html__( 'Price filter list', 'basel' ),
				'slug' => 'basel-price-filter',
			 );
			...
			$args['fields'] = array(
				array(
					'id'	=> 'title',
					'type'  => 'text',
					'std'   => esc_html__( 'Price filter', 'basel' ),
					'name' 	=> esc_html__( 'Title', 'basel' )
				),
				array(
					'id'	      => 'show_ranges',
					'type'        => 'checkbox',
					'name' 	      => esc_html__( 'Show empty price ranges', 'basel' ),
					'description' => esc_html__( 'May increase this widget performance if you have a lot of products in your store.', 'basel' ),
				),
			);

			$this->_check_ranges = apply_filters('basel_check_ranges_price_filter', true);
					...
					<div class="basel-price-filter">
						<ul>
							<?php foreach ($links as $link) : ?>
								<li>
									<a rel="nofollow" href="<?php echo esc_url( $link['href'] ); ?>" class="<?php echo esc_attr( $link['class'] ); ?>"><?php echo wp_kses( $link['title'], basel_get_allowed_html() ); ?></a>
								</li>
							<?php endforeach; ?>
						</ul>
					</div>
		...
		private function generate_price_links( $min, $max, $min_price, $max_price, $show_ranges ) {
			...
			$link = basel_shop_page_link( true );
