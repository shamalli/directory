...
if ( ! function_exists( 'basel_font_icon_preload' ) ) {
	function basel_font_icon_preload() {
		$version = basel_get_theme_info( 'Version' );
		?>
		<?php if ( basel_get_opt( 'font_icon_woff_preload' ) ) : ?>
			<link rel="preload" as="font" href="<?php echo esc_url( BASEL_THEME_DIR ); ?>/fonts/basel-font.woff?v=<?php echo esc_attr( $version ); ?>" type="font/woff" crossorigin>
		<?php endif; ?>

		<?php if ( basel_get_opt( 'font_icon_woff2_preload' ) ) : ?>
			<link rel="preload" as="font" href="<?php echo esc_url( BASEL_THEME_DIR ); ?>/fonts/basel-font.woff2?v=<?php echo esc_attr( $version ); ?>" type="font/woff2" crossorigin>
		<?php endif; ?>

		<?php if ( basel_get_opt( 'font_simple_icon_woff_preload' ) ) : ?>
			<link rel="preload" as="font" href="<?php echo esc_url( BASEL_THEME_DIR ); ?>/fonts/Simple-Line-Icons.woff?v=<?php echo esc_attr( $version ); ?>" type="font/woff" crossorigin>
		<?php endif; ?>

		<?php if ( basel_get_opt( 'font_simple_icon_woff2_preload' ) ) : ?>
			<link rel="preload" as="font" href="<?php echo esc_url( BASEL_THEME_DIR ); ?>/fonts/Simple-Line-Icons.woff2?v=<?php echo esc_attr( $version ); ?>" type="font/woff2" crossorigin>
		<?php endif; ?>
		<?php
	}

	add_action( 'wp_head', 'basel_font_icon_preload' );
}
...
if( !function_exists( 'basel_get_logo' ) ) {
	function basel_get_logo() {
		...
		?>
		<a href="<?php echo esc_url( home_url('/') ); ?>" rel="home" aria-label="<?php esc_attr_e( 'Logo', 'basel' ); ?>"><img src="<?php echo esc_url( $logo_src ); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>"></a>
		<?php 
	}
}
...
if( ! function_exists( 'basel_page_top_part' ) ) {
	function basel_page_top_part() {
		...
		<?php elseif( basel_is_pjax() ): ?>
			<title><?php echo esc_html( basel_get_document_title() ); ?></title>
		<?php endif ?>
			...
			<script type="text/javascript">
				jQuery( document ).ready(function( $ ) {
					...
					var options = {
						...
						<?php if ( 'yes' === $autoplay ): ?>
							autoplayHoverPause: <?php echo apply_filters( 'basel_autoplay_hover_pause', 'false' ); ?>,
						<?php endif; ?>
...
if( ! function_exists( 'basel_page_title' ) ) {
	...
	function basel_page_title() {
		...
		$custom_page_title_tag = basel_get_opt( 'page_title_tag' );
		...
		basel_enqueue_inline_style( 'page-title' );
				...
				<div class="page-title <?php echo esc_attr( $title_class ); ?>" style="<?php echo esc_attr( $style ); ?>">
					<div class="container">
						<header class="entry-header">
							...
							<<?php echo esc_attr( $title_tag ); ?> class="entry-title"><?php the_title(); ?></<?php echo esc_attr( $title_tag ); ?>>
		...
		if( is_singular( 'page' ) && ( ! $page_for_posts || ! is_page( $page_for_posts ) ) ):
			...
			?>
				<div class="page-title <?php echo esc_attr( $title_class ); ?>" style="<?php echo esc_attr( $style ); ?>">
					<div class="container">
						<header class="entry-header">
							<?php if( $page_title ): ?><<?php echo esc_attr( $title_tag ); ?> class="entry-title"><?php echo esc_html( $title ); ?></<?php echo esc_attr( $title_tag ); ?>><?php endif; ?>
		...
		if( is_post_type_archive( 'portfolio' ) || is_singular( 'portfolio' ) || is_tax( 'project-cat' ) ):
			...
			?>
				<div class="page-title <?php echo esc_attr( $title_class ); ?> title-blog" style="<?php echo esc_attr( $style ); ?>">
					<div class="container">
						<header class="entry-header">
							<?php if( $page_title ): ?><<?php echo esc_attr( $title_tag ); ?> class="entry-title"><?php echo wp_kses( $title, basel_get_allowed_html() ); ?></<?php echo esc_attr( $title_tag ); ?>><?php endif; ?>
							<?php if( $breadcrumbs ) basel_current_breadcrumbs( 'pages' ); ?>
						</header><!-- .entry-header -->
					</div>
				</div>
					...
					<div class="page-title <?php echo esc_attr( $title_class ); ?> title-shop" style="<?php echo esc_attr( $style ); ?>">
						<div class="container">
							<div class="nav-shop">
								...
								<?php if ( $shop_title ): ?>
									<<?php echo esc_attr( $title_tag ); ?> class="entry-title"><?php woocommerce_page_title(); ?></<?php echo esc_attr( $title_tag ); ?>>
								<?php endif ?>
...
if( ! function_exists( 'basel_breadcrumbs' ) ) {
	function basel_breadcrumbs() {
		...
		if (is_home() || is_front_page()) {
			...
			echo '<div class="breadcrumbs" xmlns:v="https://schema.org/">';
			if ($show_home_link == 1) {
...
if( ! function_exists( 'basel_cookies_popup' ) ) {
	...
	function basel_cookies_popup() {
		...
		basel_enqueue_inline_style( 'opt-cookies-popup' );
			...
			<div class="basel-cookies-popup">
				<div class="basel-cookies-inner">
					...
					<div class="cookies-buttons">
						<a href="#" rel="nofollow" class="cookies-accept-btn"><?php _e( 'Accept' , 'basel' ); ?></a>
						<?php if ( $page_id ): ?>
			...
			<div class="wishlist-info-widget">
				<a href="<?php echo esc_url( basel_get_whishlist_page_url() ); ?>">
					...
					<?php if ( ! basel_get_opt( 'wishlist_hide_product_count' ) ): ?>
						<span class="wishlist-count icon-count"><?php echo esc_html( basel_get_wishlist_count() ); ?></span>
					<?php endif; ?>
				</a>
			</div>
		<?php endif;
...
if( ! function_exists( 'basel_header_block_compare' ) ) {
	function basel_header_block_compare() {
		if ( basel_woocommerce_installed() && basel_get_opt( 'header_compare' ) ): ?>
			<div class="compare-info-widget">
				<a href="<?php echo esc_url( basel_get_compare_page_url() ); ?>">
					<?php esc_html_e( 'Compare', 'basel' ); ?>
					<?php if ( ! basel_get_opt( 'compare_hide_product_count' ) ): ?>
						<span class="compare-count icon-count"><?php echo esc_html( basel_get_compare_count() ); ?></span>
					<?php endif; ?>
				</a>
			</div>
		<?php endif;
	...
	function basel_header_block_search() {
		...
		?>
			<div class="<?php echo esc_attr( $classes ); ?>">
				<a href="#" rel="nofollow" aria-label="<?php esc_html_e( 'Search', 'basel' ); ?>">
				...
				<div class="basel-search-wrapper">
					<div class="basel-search-inner">
						...
						<?php // (c)NRI: directory search form ?>
						<?php if ( class_exists( 'w2dc_plugin' ) ) : ?>
							<form role="search" class="searchform" style="margin-top: 20px;" action="<?php global $w2dc_instance; echo $w2dc_instance->directories->getDefaultDirectory()->url ?>">
								<div>
									<input type="text" name="keywords" placeholder="<?php echo esc_attr_e( 'Chercher une boutique...', 'gsmp' ); ?>" value="<?php echo esc_attr( stripslashes( w2dc_getValue( $_GET, 'what_search', w2dc_getValue( $args, 'what_search' ) ) ) ); ?>" />
		...
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			<div class="search-by-category input-dropdown">
				<div class="input-dropdown-inner">
					...
					<a href="#" rel="nofollow" data-val="0"><?php esc_html_e('Select category', 'basel'); ?></a>
					<ul class="dropdown-list" style="display:none;">
						<li style="display:none;"><a href="#" rel="nofollow" data-val="0"><?php esc_html_e('Select category', 'basel'); ?></a></li>
						<?php
							if( ! apply_filters( 'basel_show_only_parent_categories_dropdown', false ) ) {
							    ...
							    foreach ( $terms as $term ) {
							    	?>
										<li><a href="#" rel="nofollow" data-val="<?php echo esc_attr( $term->slug ); ?>"><?php echo esc_attr( $term->name ); ?></a></li>
							    	<?php
							    }
							}
						?>
...
if( ! function_exists( 'basel_header_block_categories_menu' ) ) {
	function basel_header_block_categories_menu() {
		...
		$html = '';
		$extra_classes = '';
		if ( basel_get_opt( 'more_cat_button' ) ) {
			$extra_classes .= ' basel-more-cat';
			$html .= '<li class="menu-item basel-more-cat-btn"><a href="#" rel="nofollow" class="basel-nav-link" aria-label="' . esc_attr__( 'Show more category button', 'basel' ) . '"></a></li>';
		}

		if ( $opened ) {
			$extra_classes .= ' opened-menu';
        } else {
			$extra_classes .= ' show-on-hover';
        }

		?>
			<div class="mega-navigation <?php echo esc_attr( $extra_classes ); ?>" role="navigation">
...
if( ! function_exists( 'basel_header_block_mobile_nav' ) ) {
	function basel_header_block_mobile_nav() {
		...
		$mobile_search_close_btn = basel_get_opt( 'mobile_search_close_btn', false );

		?>
			<div class="mobile-nav">
				<?php if ( $mobile_search_close_btn ) : ?>
...
if( ! function_exists( 'basel_get_header_links' ) ) {
	function basel_get_header_links( $location ) {
		...
		if ( basel_get_opt( 'header_wishlist' ) && 'mobile' === $location && ! ( ! is_user_logged_in() && basel_get_opt( 'wishlist_logged' ) ) ) {
		...
		if ( basel_get_opt( 'header_compare' ) && $location == 'mobile' ) {
			$links[] = array(
				'label' => esc_html__( 'Compare', 'basel' ),
				'url' => basel_get_compare_page_url(),
				'class' => 'compare'
			);
		}
...
if( ! function_exists( 'basel_get_sticky_social' ) ) {
	function basel_get_sticky_social() {
		...
		basel_enqueue_inline_style( 'opt-sticky-social' );
...
if( ! function_exists( 'basel_header_banner' ) ) {
	function basel_header_banner() {
		...
		basel_enqueue_inline_style( 'opt-header-banner' );
	    ...
	    <div class="header-banner color-scheme-<?php echo esc_attr( basel_get_opt( 'header_banner_color' ) ); ?>">
	        ...
	        <?php if ( basel_get_opt( 'header_close_btn' ) ): ?>
		        <a href="#" rel="nofollow" class="close-header-banner" aria-label="<?php esc_attr_e( 'Close header banner', 'basel' ); ?>"></a>
	        <?php endif; ?>
			
			<?php if ( $banner_link ): ?>
				<a href="<?php echo esc_url( $banner_link ) ?>" class="header-banner-link" aria-label="<?php esc_attr_e( 'Header banner link', 'basel' ); ?>"></a>
	        <?php endif; ?>
		...
		?>
			<div class="login-form-side">
				<div class="widget-heading">
					<span class="widget-title"><?php esc_html_e( 'Sign in', 'basel' ); ?></span>
...
if( ! function_exists( 'basel_login_form' ) ) {
	function basel_login_form( $echo = true, $action = false, $message = false, $hidden = false, $redirect = false ) {
		...
		<form method="post" class="login woocommerce-form woocommerce-form-login <?php if ( $hidden ) echo 'hidden-form'; ?>" <?php echo ( ! empty( $action ) ) ? 'action="' . esc_url( $action ) . '"' : ''; ?> <?php if ( $hidden ) echo 'style="display:none;"'; ?>>
			...
			<?php if ( class_exists( 'BASEL_Auth' ) && ( ( ! empty( $fb_app_id ) && ! empty( $fb_app_secret ) ) || ( ! empty( $goo_app_id ) && ! empty( $goo_app_secret ) ) || ( ! empty( $vk_app_id ) && ! empty( $vk_app_secret ) ) ) ): ?>			<?php basel_enqueue_inline_style( 'woo-opt-social-login' ); ?>
...
if ( ! function_exists( 'basel_sticky_toolbar_template' ) ) {
	...
	function basel_sticky_toolbar_template() {
		if ( basel_get_opt( 'maintenance_mode' ) && ! is_user_logged_in() ) {
			return;
		}
		...
		if ( ! basel_get_opt( 'sticky_toolbar' ) || ! $enabled_fields || is_admin() || defined( 'IFRAME_REQUEST' ) ) {
			return;
		}
		...
		basel_enqueue_inline_style( 'opt-bottom-toolbar' );
...
if ( !function_exists( 'basel_get_twitts' ) ) {
	function basel_get_twitts( $args = array() ) {
		...
		if( isset( $tweets ) ) { ?>
            <ul <?php echo ( isset( $args['show_avatar'] ) && $args['show_avatar'] ) ? ' class="twitter-avatar-enabled"':""; ?>>
