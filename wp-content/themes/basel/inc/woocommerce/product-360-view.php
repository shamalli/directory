...
if( ! function_exists( 'basel_360_metabox_output' ) ) {
	function basel_360_metabox_output( $post ) {
					...
					if ( ! empty( $attachments ) ) {
						foreach ( $attachments as $attachment_id ) {
							...
							echo '<li class="image" data-attachment_id="' . esc_attr( $attachment_id ) . '">
								' . $attachment . '
								<ul class="actions">
									<li><a href="#" rel="nofollow" class="delete tips" data-tip="' . esc_attr__( 'Delete image', 'basel' ) . '">' . esc_html__( 'Delete', 'basel' ) . '</a></li>
								</ul>
							</li>';
		...
		<p class="add_product_360_images hide-if-no-js">
			<a href="#" rel="nofollow" data-choose="<?php esc_attr_e( 'Add Images to Product 360 view Gallery', 'basel' ); ?>" data-update="<?php esc_attr_e( 'Add to gallery', 'basel' ); ?>" data-delete="<?php esc_attr_e( 'Delete image', 'basel' ); ?>" data-text="<?php esc_attr_e( 'Delete', 'basel' ); ?>"><?php _e( 'Add product 360 view gallery images', 'basel' ); ?></a>
		</p>
		<?php
