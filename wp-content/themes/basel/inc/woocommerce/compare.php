...
if ( ! function_exists( 'basel_add_to_compare' ) ) {
	...
	function basel_add_to_compare() {
		...
		setcookie( $cookie_name, json_encode( $products ), 0, COOKIEPATH, COOKIE_DOMAIN, basel_cookie_secure_param(), false );
...
if ( ! function_exists( 'basel_add_to_compare_btn' ) ) {
	...
	function basel_add_to_compare_btn() {
		global $product;
		echo '<div class="basel-compare-btn product-compare-button"><a class="button" rel="nofollow" href="' . esc_url(
			basel_get_compare_page_url() ) . '" data-added-text="' . esc_html__( 'Compare products', 'basel' ) . '" data-id="' . esc_attr( $product->get_id() ) . '">' . esc_html_x( 'Compare', 'add_button', 'basel' ) . '</a></div>';
	}
}
...
if ( ! function_exists( 'basel_add_to_compare_single_btn' ) ) {
	...
	function basel_add_to_compare_single_btn() {
		global $product;
		echo '<a class="basel-compare-btn button" rel="nofollow" href="' . esc_url( basel_get_compare_page_url() ) . '" data-added-text="' . esc_html__( 'Compare products', 'basel' ) . '" data-id="' . esc_attr( $product->get_id() ) . '">' . esc_html_x( 'Compare', 'add_button', 'basel' ) . '</a>';
	}
}

if ( ! function_exists( 'basel_add_to_compare_sticky_atc_btn' ) ) {
	/**
	 * Add product to comapre button on single product
	 *
	 * @since 4.4
	 */
	function basel_add_to_compare_sticky_atc_btn() {
		global $product;
		echo '<div class="basel-sticky-btn-compare"><a class="basel-tooltip" href="' . esc_url( basel_get_compare_page_url() ) . '" data-added-text="' . esc_html__( 'Compare products', 'basel' ) . '" data-id="' . esc_attr( $product->get_id() ) . '">' . esc_html_x( 'Compare', 'add_button', 'basel' ) . '</a></div>';
	}
}
...
if ( ! function_exists( 'basel_remove_from_compare' ) ) {
	...
	function basel_remove_from_compare() {
		...
		if ( empty( $products ) ) {
			setcookie( $cookie_name, false, 0, COOKIEPATH, COOKIE_DOMAIN, basel_cookie_secure_param(), false );
		...
		switch ( $type ) {
			case 'basic':
				echo '<div class="compare-basic-content">';
					echo '<a href="#" rel="nofollow" class="basel-remove-button" data-id="' . esc_attr( $product['id'] ) . '"><span class="remove-loader"></span>' . esc_html__( 'Remove', 'basel' ) . '</a>';
...
if ( ! function_exists( 'basel_compare_add_to_cart_html' ) ) {
	...
	function basel_compare_add_to_cart_html( $product ) {
		if ( ! $product || basel_get_opt( 'catalog_mode' ) ) {
			return;
		}
