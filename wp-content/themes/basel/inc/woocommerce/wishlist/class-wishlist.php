...
class Wishlist {
	...
	public function __construct( $id = false, $user_id = false, $read_only = false ) {
		...
		if ( get_queried_object_id() == basel_get_opt( 'wishlist_page' ) ) {
	...
	private function get_current_user_wishlist() {
		...
		$id = $wpdb->get_var(
			$wpdb->prepare(
				"
				SELECT ID
				FROM $wpdb->basel_wishlists_table
				WHERE user_id = %d
			",
				$this->get_user_id()
			)
	...
	public function is_table_exists() {
		global $wpdb;
		$wishlists_table_count = $wpdb->query( "SHOW TABLES LIKE '{$wpdb->basel_wishlists_table}%'" );//phpcs:ignore
	...
	public function update_count_cookie() {
		$cookie_name = 'basel_wishlist_count';
		...
		basel_set_cookie( $cookie_name, $this->get_count() );
	}
	...
	public function move_products_if_needed() {
		...
		basel_set_cookie( $cookie_name, false );
