		...
		var baselOptionsAdmin = {
			...
			selectControl: function (force_init) {
				...
				if ($select.length > 0) {
					...
					$select.each(function() {
						if ($(this).hasClass('xts-field-inited')) {
							return;
						}
						...
						if ( $(this).parents('#widget-list').length > 0 ) {
							return;
						}

						$(this).select2(select2Defaults);
						$(this).addClass('xts-field-inited');
					});
				}
			...
			typographyControlInit: function() {
				...
				$.ajax({
					...
					method  : 'POST',
					data    : {
						action: 'basel_get_theme_settings_typography_data',
						security: baselConfig.get_theme_settings_data_nonce,
					},
					dataType: 'json',
					success : function(response) {
						baselOptionsAdmin.typographyControl(response.typography);
					},
					error   : function() {
			...
			settingsSearch: function () {
				...
				$.ajax({
					...
					method  : 'POST',
					data    : {
						action: 'basel_get_theme_settings_search_data',
						security: baselConfig.get_theme_settings_data_nonce,
					},
					dataType: 'json',
					success : function(response) {
		...
		return {
			init: function () {
				$(document).ready(function () {
					...
					baselOptionsAdmin.selectControl(true);
					baselOptionsAdmin.uploadControl(true);
					baselOptionsAdmin.uploadListControl(true);
				...
				$(document).on('xts_section_changed', function() {
					setTimeout(function() {
						baselOptionsAdmin.typographyControlInit();
					});
					baselOptionsAdmin.selectControl(false);
					baselOptionsAdmin.uploadControl(false);
					baselOptionsAdmin.uploadListControl(false);
