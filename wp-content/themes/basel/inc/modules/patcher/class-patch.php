...
class Patch {
	...
	public function apply() {
		...
		if ( ! isset( $patches_map[ $this->patch_id ] ) ) {
			wp_send_json(
				array(
					'message' => esc_html__( 'Patch with this ID does\'t exist.', 'basel' ),
					'status'  => 'error',
				)
			);
		}
		...
		if ( $this->successful_write_files ) {
			...
			$patch_success[ basel_get_theme_info( 'Version' ) ][ $this->patch_id ] = true;

			update_option( 'xts_successfully_installed_patches', $patch_success );

			wp_send_json(
				array(
					'message' => esc_html__( 'Patch has been successfully applied.', 'basel' ),
					'status'  => 'success',
				)
			);
		}
		...
		wp_send_json(
			array(
				'message' => esc_html__( 'Something went wrong during patch installation. Patch can\'t be applied. Please, try again later.', 'basel' ),
				'status'  => 'error',
			)
		);
	}
