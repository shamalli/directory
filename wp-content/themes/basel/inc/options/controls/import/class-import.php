...
class Import extends Field {
	...
	public function render_control() {
		?>
			...
			<button class="xts-save-options-btn xts-btn xts-btn-primary" type="submit" name="xts-<?php echo esc_attr( $this->opt_name ); ?>-options[import-btn]" value="1"><?php esc_html_e( 'Import', 'xts-theme' ); ?></button>
			<a class="xts-save-options-btn xts-btn xts-btn-primary" href="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>?action=xts_download_export_options&secret=<?php echo esc_attr( $this->generete_secret() ); ?>"><?php esc_html_e( 'Export options', 'xts-theme' ); ?></a>
		<?php
	}
	...
	public function generete_secret() {
		return md5( md5( AUTH_KEY . SECURE_AUTH_KEY ) . '-xts-theme' );
	}
