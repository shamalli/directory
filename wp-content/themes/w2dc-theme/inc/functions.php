...
function wdt_get_default_theme_options() {
	...
	$defaults['page_header_background'] = '#428bca';
	$defaults['page_header_text_color'] = '#FFFFFF';
	$defaults['links_color'] = '#428bca';
	$defaults['links_color_hover'] = '#428bca';
	$defaults['buttons_color'] = '#428bca';
	$defaults['buttons_color_hover'] = '#428bca';
...
function wdt_get_page_featured_image() {
	...
	if ($w2rr_instance) {
		if ($review = w2rr_isReview()) {
			$image_url = $review->get_logo_url(array(W2DC_FEATURED_SIZE_WIDTH, W2DC_FEATURED_SIZE_HEIGHT));
		}
	}
