	...
	$(document).ready(function(){
		...
		jQuery.fn.scReset = function(){
			...
			if ($.fn.select2)
			{
				...
				vp_w2theme.init_fontawesome_chooser($(this).find('.vp-js-fontawesome'));
				vp_w2theme.init_select2($(this).find('.vp-js-select2'));
				vp_w2theme.init_sorter($(this).find('.vp-js-sorter'));
			}
			return $(this);
		};

		// init controls
		vp_w2theme.init_controls($('.vp-sc-main'));

		// shortcode image controls event bind
		vp_w2theme.custom_check_radio_event(".vp-sc-dialog", ".vp-checkimage .field .input label");
		vp_w2theme.custom_check_radio_event(".vp-sc-dialog", ".vp-radioimage .field .input label");
		...
		$('.vp-sc-element .vp-sc-element-heading a').bind('click.vp_w2theme_sc', function(e){
			e.preventDefault();
			...
			else
			{
				...
				$modal.find('.vp-sc-element-form').vp_w2theme_slideUp();

				if($form.exists())
				{
					$parent.addClass('active');
					$form.vp_w2theme_slideDown();
				}
				else
				{
					code = decodeEntities(code);
					$modal.trigger('vp_w2theme_insert_shortcode', code);
					$modal.trigger('reveal:close');
				}
			}
		});

		$('.vp-sc-insert').bind('click.vp_w2theme_sc_insert', function(e){
			e.preventDefault();
			var $modal  = $(this).parents('.vp-sc-dialog'),
			...
			$form.on('submit', function(e){vp_w2theme.tinyMCE_save(); e.preventDefault();});
			$form.submit();
		...
		$('.vp-sc-cancel').bind('click.vp_w2theme_sc_cancel', function(e){
			e.preventDefault();
			$('.vp-sc-element').removeClass('active');
