;(function($) {
	...
	function vp_w2theme_init_fields($elements)
	{
		$elements.each(function(){
			if($(this).parents('.tocopy').length <= 0)
			{
				vp_w2theme.init_controls($(this));
	...
	function vp_w2theme_init_groups($elements)
	{
		$elements.each(function(){
	...
	function vp_w2theme_mb_sortable()
	{
		var textareaIDs = [];
				...
				if(typeof window.KIA_metabox !== 'undefined')
				{
					...
					$(ui.item).find('.customEditor textarea').each(function(){
						if($(this).parents('.tocopy').length <= 0)
						{
							...
							textareaIDs.push(vp_w2theme.jqid(this.id));
						}
					});
				}
			},
	...
	$(document).ready(function () {
		vp_w2theme_init_fields(jQuery('.vp-metabox .vp-field'));
		vp_w2theme_init_groups(jQuery('.vp-metabox .vp-meta-group'));
		process_binding(bindings);
		process_items_binding(items_binding);
		process_dependency(dependencies);
		vp_w2theme_mb_sortable();
	});

	vp_w2theme.is_multianswer = function(type){
	...
	vp_w2theme.custom_check_radio_event(".vp-metabox", ".vp-field.vp-checkimage .field .input label");
	vp_w2theme.custom_check_radio_event(".vp-metabox", ".vp-field.vp-radioimage .field .input label");
	...
	$('#post').on( 'submit', function(e){
		...
		errors = vp_w2theme.fields_validation_loop(validation);
	...
	function process_binding(bindings)
	{
		for (var i = 0; i < bindings.length; i++)
		{
			...
			{
				vp_w2theme.binding_event(ids, j, field, func, '.vp-metabox', 'metabox');
			}
		}
	}
	...
	function process_items_binding(items_binding)
	{
		for (var i = 0; i < items_binding.length; i++)
		{
			...
			for (j = 0; j < ids.length; j++)
			{
				vp_w2theme.items_binding_event(ids, j, field, func, '.vp-metabox', 'metabox');
			}
		}
	}
	...
	function process_dependency(dependencies)
	{
		for (var i = 0; i < dependencies.length; i++)
		{
			...
			{
				var $source = jQuery(vp_w2theme.jqid(field.source));
				if($source.parents('.wpa_group').length > 0)
				{
					prefix = jQuery(vp_w2theme.jqid(field.source)).parents('.wpa_group').first().attr('id');
				}
				else
				{
					// get the closest 'postbox' class parent id
					prefix = jQuery(vp_w2theme.jqid(field.source)).parents('.postbox').attr('id');
					// strip the '_metabox'
			...
			for (j = 0; j < ids.length; j++)
			{
				vp_w2theme.dependency_event(ids, j, field, func, '.vp-metabox');
			}
		}
	}
	...
	$.wpalchemy.on('wpa_copy', function(event, clone){
		...
		vp_w2theme_init_fields(clone.find('.vp-field'));
		vp_w2theme_init_groups(clone.find('.vp-meta-group'));
