...
// hidding plugin
jQuery.fn.vp_w2theme_slideUp = function(callback){
	var $this = this;
...
jQuery.fn.vp_w2theme_slideDown = function(callback){
	var $this = this;
...
jQuery.fn.vp_w2theme_fadeOut = function(callback){
	var $this = this;
...
jQuery.fn.vp_w2theme_fadeIn = function(callback){
	var $this = this;
	if($this.hasClass('vp-hide'))
	{
...
jQuery.fn.vp_w2theme_toggle = function(callback){
	var $this = this;
	if($this.hasClass('vp-hide'))
	{
...
vp_w2theme.isNumber = function(n) {
  return !isNaN(parseFloat(n)) && isFinite(n);
};
vp_w2theme.parseOpt = function(optString) {
	...
	for (var i = 0; i < optString.length; i++)
	{
		...
		if (optString[i] == ')')
		{
			...
			tempArr = temp.split(':');
			opt[tempArr[0]] = vp_w2theme.isNumber(tempArr[1]) ? parseFloat(tempArr[1]) : tempArr[1];
		}
	}
	return opt;
};

vp_w2theme.wp_ext2type = function ( ext ) {
	var ext2type = {
...
vp_w2theme.get_url_extension = function(url){
...
vp_w2theme.jqid = function(id) {
	return '#' + id.replace(/([:\.\[\]])/g,'\\$1');
};

vp_w2theme.jqidwild = function(id) {
	id = id.replace(/([:\.\[\]])/g,'\\$1');
	id = '[id*=' + id + ']';
	return id;
};

vp_w2theme.jqname = function(name) {
	return '[name="' + name + '"]';
};

vp_w2theme.jqnamewild = function(name) {
	return '[name*="' + name + '"]';
};

vp_w2theme.thejqid = function(id, thecase) {
	if(thecase === 'option')
		return vp_w2theme.jqid(id);
	if(thecase === 'metabox')
		return vp_w2theme.jqidwild(id);
	return id;
};

vp_w2theme.thejqname = function(name, thecase) {
	if(thecase === 'option')
		return vp_w2theme.jqname(name);
	if(thecase === 'metabox')
		return vp_w2theme.jqnamewild(name);
	return name;
};

vp_w2theme.validateAlphabet = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vp_w2theme_wp.alphabet_validatable) == -1) { return true; }
	var regex = new RegExp(/^[A-Z\s]+$/i);
	return regex.test(val);
};

vp_w2theme.validateAlphaNumeric = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vp_w2theme_wp.alphanumeric_validatable) == -1) { return true; }

	var regex = new RegExp(/^[A-Z0-9]+$/i);
	return regex.test(val);
};

vp_w2theme.validateNumeric = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vp_w2theme_wp.numeric_validatable) == -1) { return true; }

	var regex = new RegExp(/^[-+]?[0-9]*\.?[0-9]+$/);
	return regex.test(val);
};

vp_w2theme.validateEmail = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vp_w2theme_wp.email_validatable) == -1) { return true; }

	var regex = new RegExp(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i);
	return regex.test(val);
};

vp_w2theme.validateURL = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vp_w2theme_wp.url_validatable) == -1) { return true; }

	var regex = new RegExp(/^(ftp|http|https):\/\/[^ "]+$/i);
	return regex.test(val);
};

vp_w2theme.validateMaxLength = function(type, val, n) {
	// ignore array
	if (jQuery.inArray(type, vp_w2theme_wp.maxlength_validatable) != -1) { return true; }

	return (val.length <= n) ? true : false;
};

vp_w2theme.validateMinLength = function(type, val, n) {
	// ignore array
	if (jQuery.inArray(type, vp_w2theme_wp.minlength_validatable) != -1) { return true; }

	return (val.length >= n) ? true : false;
};

vp_w2theme.validateRequired = function(type, val) {
...
vp_w2theme.fields_validation_loop = function(fields){
	...
	{
		var field   = fields[i],
		    $tr     = jQuery(vp_w2theme.jqid(field.name)),
		...
		for (var j = 0; j < rules.length; j++)
		{
			...
			{
				case 'alphabet':
					if (!vp_w2theme.validateAlphabet(type, val)) { res = vp_w2theme_wp.val_msg.alphabet.format(); }
					break;
				case 'alphanumeric':
					if (!vp_w2theme.validateAlphaNumeric(type, val)) { res = vp_w2theme_wp.val_msg.alphanumeric.format(); }
					break;
				case 'numeric':
					if (!vp_w2theme.validateNumeric(type, val)) { res = vp_w2theme_wp.val_msg.numeric.format(); }
					break;
				case 'email':
					if (!vp_w2theme.validateEmail(type, val)) { res = vp_w2theme_wp.val_msg.email.format(); }
					break;
				case 'url':
					if (!vp_w2theme.validateURL(type, val)) { res = vp_w2theme_wp.val_msg.url.format(); }
					break;
				case 'maxlength':
					n = rule.substring(q1 + 1, q2);
					if (!vp_w2theme.validateMaxLength(type, val, n)) { res = vp_w2theme_wp.val_msg.maxlength.format(n); }
					break;
				case 'minlength':
					n = rule.substring(q1 + 1, q2);
					if (!vp_w2theme.validateMinLength(type, val, n)) { res= vp_w2theme_wp.val_msg.minlength.format(n); }
					break;
				case 'maxselected':
					n = rule.substring(q1 + 1, q2);
					if (!vp_w2theme.validateMaxLength(type, val, n)) { res = vp_w2theme_wp.val_msg.maxselected.format(n); }
					break;
				case 'minselected':
					n = rule.substring(q1 + 1, q2);
					if (!vp_w2theme.validateMinLength(type, val, n)) { res= vp_w2theme_wp.val_msg.minselected.format(n); }
					break;
				case 'required':
					if (!vp_w2theme.validateRequired(type, val)) { res = vp_w2theme_wp.val_msg.required.format(); }
					break;
			}
...
vp_w2theme.custom_check_radio_event = function(parent, selector){
...
vp_w2theme.binding_action =	function(ids, field, func, thecase) {
	var $source_tr = jQuery(vp_w2theme.jqid(field.source)),
	    $source    = jQuery('[name="' + field.source + '"]'),
	    values     = [];

	for (var i = 0; i < ids.length; i++)
	{
		var val = jQuery(vp_w2theme.thejqname(ids[i], thecase)).validationVal();
	...
	var data = {
		action   : 'vp_w2theme_ajax_wrapper',
		func     : func,
		params   : values
	};
	...
	if (field.type == 'vp-html') {
		$loader.vp_w2theme_fadeIn();
	} else {
		$input.vp_w2theme_fadeOut(function(){
			$loader.vp_w2theme_fadeIn();
		});
	}

	jQuery.post(ajaxurl, data, function(response) {
		$loader.vp_w2theme_fadeOut(function(){
			$input.vp_w2theme_fadeIn();
		});
		if (response.status)
		{
			var data;
			switch(field.type)
			{
				...
				case 'vp-html':
					if(response.data !== null)
					{
						jQuery(vp_w2theme.jqid(field.source + '_dom')).html(response.data);
						jQuery(vp_w2theme.jqname(field.source)).val(response.data);
					}
					break;
				default:
					$source = jQuery(vp_w2theme.jqname(field.source));
...
vp_w2theme.binding_event = function(ids, idx, field, func, parent, thecase)
{
	var change    = ['vp-select', 'vp-checkbox', 'vp-checkimage', 'vp-radiobutton', 'vp-radioimage', 'vp-multiselect', 'vp-toggle', 'vp-upload'],
	    ...
	    name      = vp_w2theme.thejqname(ids[idx], thecase),
	    dest_type = jQuery(vp_w2theme.thejqid(ids[idx], thecase)).attr('data-vp-type');

	if(jQuery.inArray(dest_type, change) !== -1 )
	{
		jQuery(parent).delegate(name, 'change', function(){vp_w2theme.binding_action(ids, field, func, thecase);});
	}
	else if(jQuery.inArray(dest_type, typing) !== -1 )
	{
		jQuery(name).typing({
			stop: function(event, $elem){
				vp_w2theme.binding_action(ids, field, func, thecase);
			},
			delay: 400
		});
	}
...
// vafpress binding related functions
vp_w2theme.items_binding_action =	function(ids, field, func, thecase) {
	var $source_tr = jQuery(vp_w2theme.jqid(field.source)),
	    $source    = jQuery('[name="' + field.source + '"]'),
	    values     = [];
	...
	for (var i = 0; i < ids.length; i++)
	{
		values.push(jQuery(vp_w2theme.thejqname(ids[i], thecase)).validationVal());
	}
		...
		if (response.status)
		{
			...
			switch(field.type)
			{
				...
				case 'vp-checkbox':
					$source = $input;
					$source.empty();
					$source = jQuery(vp_w2theme.jqid(field.source)).find('.input');
					response.data !== null && jQuery.each(response.data, function(key, value) {
...
vp_w2theme.items_binding_event = function(ids, idx, field, func, parent, thecase)
{
	var change    = ['vp-select', 'vp-checkbox', 'vp-checkimage', 'vp-radiobutton', 'vp-radioimage', 'vp-multiselect', 'vp-toggle', 'vp-upload'],
	    typing    = ['vp-textbox', 'vp-slider', 'vp-color', 'vp-date'],
	    name      = vp_w2theme.thejqname(ids[idx], thecase),
	...
	if(jQuery.inArray(dest_type, change) !== -1 )
	{
		jQuery(parent).delegate(name, 'change', function(){vp_w2theme.items_binding_action(ids, field, func, thecase);});
	}
	else if(jQuery.inArray(dest_type, typing) !== -1 )
	{
		jQuery(name).typing({
			stop: function(event, $elem){
				vp_w2theme.items_binding_action(ids, field, func, thecase);
			},
			delay: 400
		});
	}
...
vp_w2theme.dependency_action =	function(ids, field, func) {
	...
	for (var i = 0; i < ids.length; i++)
	{
		targets.push(jQuery(vp_w2theme.jqid(ids[i])));
	...
	jQuery.post(ajaxurl, data, function(response) {
		...
		if (response.status)
		{
			if(response.data)
			{
				...
				$source_tr.vp_w2theme_fadeIn();
			}
			else
			{
				$source_tr.addClass('vp-dep-inactive');
				$source_tr.vp_w2theme_fadeOut();
			}
		}
	}, 'JSON');
};

vp_w2theme.dependency_event = function(ids, idx, field, func, parent){

	var change    = ['vp-select', 'vp-checkbox', 'vp-checkimage', 'vp-radiobutton', 'vp-radioimage', 'vp-multiselect', 'vp-toggle', 'vp-upload'],
	    typing    = ['vp-textbox', 'vp-slider', 'vp-color', 'vp-date'],
	    name      = vp_w2theme.thejqname(ids[idx], 'option'),
	    dest_type = jQuery(vp_w2theme.thejqid(ids[idx], 'option')).attr('data-vp-type');

	if(jQuery.inArray(dest_type, change) !== -1 )
	{
		jQuery(parent).delegate(name, 'change', function(){vp_w2theme.dependency_action(ids, field, func);});
	}
	else if(jQuery.inArray(dest_type, typing) !== -1 )
	{
		jQuery(name).typing({
			stop: function(event, $elem){
				vp_w2theme.dependency_action(ids, field, func);
			},
			delay: 400
		});
	}
...
function theValidate(elem, options, $slider)
{
	...
	if(val === '')
		return;
	if (!vp_w2theme.validateNumeric('vp-textbox', val))
	{
...
vp_w2theme.init_slider = function($elements)
{
	if (jQuery.fn.slider)
	{
		$elements.each(function(i, el) {
			...
			options = vp_w2theme.parseOpt(options.opt);
			options.range = 'min';
...
vp_w2theme.upload_callback = function() {};

if ( vp_w2theme_wp.use_new_media_upload )
{
	var _orig_send_attachment = wp.media.editor.send.attachment,
	    _orig_send_link       = wp.media.editor.send.link,
	    _orig_send_to_editor  = window.send_to_editor;

	vp_w2theme.upload_callback = function(e) {
...
function get_url_info(html)
{
	...
	if ($el.prop('tagName') == 'A')
	{
		...
		ext     = vp_w2theme.get_url_extension(imgurl);
		type    = vp_w2theme.wp_ext2type(ext);
		iconurl = imgurl;
		if(type !== 'image')
		{
			iconurl = vp_w2theme_wp.wp_include_url + 'images/crystal/' + type + '.png' ;
		}
	}
...
vp_w2theme.remove_upload_callback = function(e) {
	var $this    = jQuery(this),
	    $input   = $this.parent('.buttons').prev('input'),
	    $preview = $this.parent('.buttons').siblings('.image').find('img');
...
jQuery(document).on('click', '.vp-js-upload', vp_w2theme.upload_callback);
jQuery(document).on('click', '.vp-js-remove-upload', vp_w2theme.remove_upload_callback);

vp_w2theme.init_colorpicker = function($elements)
{
...
vp_w2theme.init_datepicker = function($elements)
{
...
vp_w2theme.init_controls = function($parent)
{
	// init date picker
	vp_w2theme.init_datepicker($parent.find('.vp-js-datepicker'));
	vp_w2theme.init_fontawesome_chooser($parent.find('.vp-js-fontawesome'));
	vp_w2theme.init_select2($parent.find('.vp-js-select2'));
	vp_w2theme.init_sorter($parent.find('.vp-js-sorter'));
	vp_w2theme.init_colorpicker($parent.find('.vp-js-colorpicker'));
	vp_w2theme.init_slider($parent.find('.vp-js-slider'));
	vp_w2theme.init_ace_editor($parent.find('.vp-js-codeeditor'));
	vp_w2theme.init_wpeditor($parent.find('.vp-js-wpeditor'));
};

// Fontawesome Chooser
vp_w2theme.init_fontawesome_chooser = function($elements)
{
	if (jQuery.fn.select2)
	{
		if($elements.length <= 0)
			return;
		var format = function vp_w2theme_fontawesome_chooser_format(icon){
			return '<span class="fontawesome"><i class="w2theme-fa ' + icon.id + '"></i>' + icon.text + '</span>';
		};
		$elements.select2({
			formatResult: format,
			...
			allowClear: true,
			placeholder: vp_w2theme_wp.ctrl_msg.fac_placeholder
		});
	}
};

// Select2
vp_w2theme.init_select2 = function($elements)
{
	if (jQuery.fn.select2)
	{
		if($elements.length <= 0)
			return;
		$elements.select2({allowClear: true, placeholder: vp_w2theme_wp.ctrl_msg.select2_placeholder});
	}
};

// Sorter
vp_w2theme.init_sorter = function($elements)
{
...
// Tipsy
vp_w2theme.init_tipsy = function()
{
	if (jQuery.fn.tipsy)
	{
...
// Init Sorter
vp_w2theme.init_ace_editor = function($elements)
{
...
vp_w2theme.init_wpeditor = function($elements)
{
	if(typeof window.KIA_metabox !== 'undefined')
	{
		if($elements.length <= 0)
...
vp_w2theme.tinyMCE_save = function()
{
