...
class VP_W2THEME_ShortcodeGenerator
{
	...
	{
		$this->main_image     = VP_W2THEME_PUBLIC_URL . '/img/vp_w2theme_shortcode_icon.png';
		$this->sprite_image   = VP_W2THEME_PUBLIC_URL . '/img/vp_w2theme_shortcode_icon_sprite.png';
	...
	{
		$post_type  = VP_W2THEME_Metabox::_get_current_post_type();
	...
	public function can_output()
	{
		...
		// if in post / page
		if( VP_W2THEME_Metabox::_is_post_or_page() )
		{
			// then consider the types
			if( !in_array("*", $this->types) ) // if wildcard exists, then always shows
				$can &= in_array(VP_W2THEME_Metabox::_get_current_post_type(), $this->types);
			else
				$can &= true;
		}
		else
	...
	function print_form($attributes)
	{
		...
		{
			// create the object
			$make           = VP_W2THEME_Util_Reflection::field_class_from_type($attr['type']);
			// prefix name
		...
		<div class="vp-sc-action">
			<button class="vp-sc-insert button"><?php _e('Insert', 'vp_w2theme_textdomain'); ?></button>
			<button class="vp-sc-cancel button"><?php _e('Cancel', 'vp_w2theme_textdomain') ?></button>
		</div>
		<?php
	}
	...
	{
		if( VP_W2THEME_Metabox::_is_post_or_page() && !current_user_can( 'edit_posts' ) &&
	...
	public static function register_buttons($buttons)
	{
		...
		{
			if( $sg->can_output() )
				$vp_w2theme_buttons[] = $sg->name;
		}
		$buttons = array_merge($buttons, $vp_w2theme_buttons);
		return $buttons;
	}

	public static function add_buttons($plugin_array)
	{
		$plugin_array['vp_w2theme_sc_button'] = VP_W2THEME_PUBLIC_URL .'/js/shortcodes.js';
		foreach (self::$pool as $sg)
		{
			if( $sg->can_output() )
				$plugin_array[$sg->name] = VP_W2THEME_PUBLIC_URL .'/js/dummy.js';
		}
		return $plugin_array;
	}
