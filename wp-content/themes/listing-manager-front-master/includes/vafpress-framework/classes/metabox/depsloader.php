...
class VP_W2THEME_Metabox_Depsloader
{

	/**
	 * ARRAY OF VP_W2THEME_METABOX_ALCHEMY OBJECT
	 * @var [type]
	 */
	private $things;
	...
	{
		$vp_w2theme_metabox_used = false;
		$metaboxes = $this->things;

		$result = array(
			'scripts'              => array(),
			'styles'               => array(),
			'localize_name'        => 'vp_w2theme_mb',
		...
		$script_always = VP_W2THEME_Util_Config::instance()->load('dependencies', 'scripts.always');
		$style_always  = VP_W2THEME_Util_Config::instance()->load('dependencies', 'styles.always');
		...
		if(is_array($metaboxes)) foreach ($metaboxes as $key => $metabox)
		{
			if($metabox->can_output())
			{
				...
				$vp_w2theme_metabox_used = true;
			}

			if($vp_w2theme_metabox_used)
			{
