...
class VP_W2THEME_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', 'vp_w2theme_textdomain' ),
			'menu_label'            => __( 'Vafpress Options', 'vp_w2theme_textdomain' ),
			'priority'              => 10,
		), $configs);

		// options config filter
		$configs = apply_filters('vp_w2theme_option_configuration_array'  , $configs, $configs['option_key']);
		$configs = apply_filters('vp_w2theme_option_configuration_array-' . $configs['option_key'], $configs);
		...
		else throw new Exception(__( 'Option Key is required', 'vp_w2theme_textdomain' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', 'vp_w2theme_textdomain' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', 'vp_w2theme_textdomain' ), 1);
		...
		add_action('vp_w2theme_option_first_activation', array($this, 'initial_db_setup'));

		// check if option key not existed init data from default values
		$options = get_option( $this->get_option_key() );
		if( $options === FALSE )
		{
			do_action('vp_w2theme_option_first_activation');
		}
		...
		// setup ajax
		add_action('wp_ajax_vp_w2theme_ajax_' . $this->get_option_key() . '_export_option', array($this, 'ajax_export_option'));
		add_action('wp_ajax_vp_w2theme_ajax_' . $this->get_option_key() . '_import_option', array($this, 'ajax_import_option'));
		add_action('wp_ajax_vp_w2theme_ajax_' . $this->get_option_key() . '_save'         , array($this, 'ajax_save'));
		add_action('wp_ajax_vp_w2theme_ajax_' . $this->get_option_key() . '_restore'      , array($this, 'ajax_restore'));
	...
	{
		VP_W2THEME_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'vp_w2theme_textdomain'), false);
	}

	public function enqueue_scripts_and_styles()
	{	
		$opt_loader = VP_W2THEME_WP_Loader::instance();
		...
		$opt_loader->add_js_data( 'vp-option', 'custom_local.SAVE_SUCCESS', VP_W2THEME_Option_Control_Set::SAVE_SUCCESS );
		$opt_loader->add_js_data( 'vp-option', 'custom_local.SAVE_NOCHANGES', VP_W2THEME_Option_Control_Set::SAVE_NOCHANGES );
		$opt_loader->add_js_data( 'vp-option', 'custom_local.SAVE_FAILED', VP_W2THEME_Option_Control_Set::SAVE_FAILED );
	}
	...
	function save_and_reinit()
	{
		...
		do_action('vp_w2theme_option_save_and_reinit', $opt, $result['status'], $this->get_option_key());

		// option key specific save and re-init action hook
		do_action('vp_w2theme_option_save_and_reinit-' . $this->get_option_key(), $opt, $result['status']);
	...
	function ajax_save()
	{
		$result = $this->vp_w2theme_verify_nonce();
		...
		if($result['status'])
		{
			...
			$option  = VP_W2THEME_Util_Array::unite( $option, 'name', 'value' );
			...
			do_action('vp_w2theme_option_before_ajax_save', $opt);

			// save and re-init options
			$result = $this->save_and_reinit();

			// after ajax save action hook
			do_action('vp_w2theme_option_after_ajax_save', $opt, $old_opt, $result['status'], $this->get_option_key());

			// option key specific after ajax save action hook
			do_action('vp_w2theme_option_after_ajax_save-' . $this->get_option_key(), $opt, $old_opt, $result['status']);
		}
	...
	function ajax_restore()
	{
		...
		if( $result['status'] )
		{
			...
			// before ajax save action hook
			do_action('vp_w2theme_option_before_ajax_restore', $options);
			...
			do_action('vp_w2theme_option_after_ajax_restore', $options, $old_opt, $result['status'], $this->get_option_key());

			// after ajax restore action hook
			do_action('vp_w2theme_option_after_ajax_restore-' . $this->get_option_key(), $options, $old_opt, $result['status']);
		}
	...
	{
		global $vp_w2theme_set, $vp_w2theme_config;
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", 'vp_w2theme_textdomain');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", 'vp_w2theme_textdomain');
				}
			}
		}

		// after ajax import action hook
		do_action('vp_w2theme_option_after_ajax_import', $options, $old_opt, $result['status'], $this->get_option_key());

		// after ajax import action hook
		do_action('vp_w2theme_option_after_ajax_import-' . $this->get_option_key(), $options, $old_opt, $result['status']);
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> __("Successful", 'vp_w2theme_textdomain'),
				'option' => $sr_options,
			);
		}

		// after ajax export action hook
		do_action('vp_w2theme_option_after_ajax_export', $db_options, $sr_options, $result['status'], $this->get_option_key());

		// after ajax export action hook
		do_action('vp_w2theme_option_after_ajax_export-' . $this->get_option_key(), $db_options, $sr_options, $result['status']);
	...
	function vp_w2theme_verify_nonce()
	{
		$nonce  = $_POST['nonce'];
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", 'vp_w2theme_textdomain');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access", 'vp_w2theme_textdomain');
		}
		return $result;
	}
	...
	function initial_db_setup()
	{
		...
		do_action('vp_w2theme_option_before_db_init', $opt);

		// save to db
		$result = $this->save_and_reinit();

		// after db options db action hook
		do_action('vp_w2theme_option_after_db_init', $opt, $result['status'], $this->get_option_key());
		do_action('vp_w2theme_option_after_db_init-' . $this->get_option_key(), $opt, $result['status']);
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'vp_w2theme_textdomain' ), 1);

		$parser = new VP_W2THEME_Option_Parser();
		...
		{
			// setup utility menu
			$util_menu = new VP_W2THEME_Option_Control_Group_Menu();
			$util_menu->set_title(__('Utility', 'vp_w2theme_textdomain'));
			$util_menu->set_name('menu_util');
			$util_menu->set_icon('font-awesome:fa-ambulance');

			// setup restore default section
			$restore_section = new VP_W2THEME_Option_Control_Group_Section();
			$restore_section->set_title(__('Restore Default', 'vp_w2theme_textdomain'));
			$restore_section->set_name('section_restore');

			// setup restore button
			$restore_button = new VP_W2THEME_Option_Control_Field_Restore();
			$restore_section->add_field($restore_button);

			// setup exim section
			$exim_section = new VP_W2THEME_Option_Control_Group_Section();
			$exim_section->set_title(__('Export/Import', 'vp_w2theme_textdomain'));
			$exim_section->set_name('section_exim');

			// setup exim field
			$exim_field = new VP_W2THEME_Option_Control_Field_ImpExp();
	...
	// @todo return `vp_w2theme_option` like function
