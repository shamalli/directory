...
class VP_W2THEME_Option_Control_Set
{
	...
	public function render()
	{
		// Setup data
		$data = array('set' => $this);
		return VP_W2THEME_View::instance()->load('option/set', $data);
	}
	...
	public function get_fields($include_section = false)
	{
		if(!function_exists('loop_controls'))
		{
			function loop_controls($menu, $include_section)
			{
				...
				{
					if( get_class($control) === 'VP_W2THEME_Option_Control_Group_Section' )
					{
						if($include_section)
						{
						...
						{
							if( VP_W2THEME_Util_Reflection::field_type_from_class(get_class($field)) != 'impexp' )
							{
					...
					{
						if( VP_W2THEME_Util_Reflection::field_type_from_class(get_class($control)) != 'impexp' )
						{
							$fields[$control->get_name()] = $control;
						}
					}
	...
	public function get_field_types()
	{
		...
		{
			$type = VP_W2THEME_Util_Reflection::field_type_from_class(get_class($field));
	...
	public function normalize_values($opt_arr)
	{
		...
		foreach ($opt_arr as $key => $value)
		{
			...
			{
				$is_multi = VP_W2THEME_Util_Reflection::is_multiselectable($fields[$key]);
	...
	public function save($option_key)
	{
		...
		do_action('vp_w2theme_option_set_before_save', $opt);

		if(update_option($option_key, $opt))
		{
			$result['status']  = true;
			$result['code']    = self::SAVE_SUCCESS;
			$result['message'] = __('Saving successful', 'vp_w2theme_textdomain');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = __('Saving failed', 'vp_w2theme_textdomain');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = __('No changes made', 'vp_w2theme_textdomain');
			}
		}

		do_action('vp_w2theme_option_set_after_save', $curr_opt, $result['status'], $option_key);
	...
	public function populate_values($opt, $force_update = false)
	{
		...
		foreach ( $fields as $field )
		{
			$is_multi = VP_W2THEME_Util_Reflection::is_multiselectable($field);
