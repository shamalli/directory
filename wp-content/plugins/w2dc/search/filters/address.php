...
function w2dc_query_args_address($q_args, $args) {
	...
	if ((wcsearch_get_query_string("address") && w2dc_do_follow_get_params($args)) || !empty($args['address']) || !empty($args['start_address']) || (!empty($args['start_latitude']) && !empty($args['start_longitude']))) {
		...
		if ($radius) {
			...
			if (!is_wp_error($result) && is_array($result) && is_numeric($result[0]) && is_numeric($result[1])) {
				...
				$dLat = sprintf('((map_coords_1-%f)*PI()/180)', $result[1]);
				$dLong = sprintf('((map_coords_2-%f)*PI()/180)', $result[0]);
			...
			$where_sql_array[] = $wpdb->prepare("(address_line_1 LIKE %s OR address_line_2 LIKE %s OR zip_or_postal_index LIKE %s)", '%' . $wpdb->esc_like($address) . '%', '%' . $wpdb->esc_like($address) . '%', '%' . $wpdb->esc_like($address) . '%');
