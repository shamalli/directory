...
class w2dc_updater {
	...
	public function support_checker_response($date) {
		...
		if ($date > time()) {
			return '<p class="w2dc-license-support-checker-active">' . esc_html__("Your support is active till ", "w2dc") . date_i18n(w2dc_getDateFormat(), intval($date)) . '. ' . esc_html__("Please follow the link if you wish", "w2dc") . ' ' . '<a href="' . w2dc_getUpdateSupportLink() . '" target="_blank">' . esc_html__("to renew", "w2dc") . '</a></p>';
		} else {
			return '<p class="w2dc-license-support-checker-expired">' . esc_html__("Your support has expired since ", "w2dc") . date_i18n(w2dc_getDateFormat(), intval($date)) . '. ' . esc_html__("Please follow the link", "w2dc") . ' ' . '<a href="' . w2dc_getUpdateSupportLink() . '" target="_blank">' . esc_html__("to renew", "w2dc") . '</a></p>';
		}
	}
	...
	public function showUpgradeMessage($plugin_data, $response) {
		if (empty($response->package)) {
			echo sprintf(wp_kses(__('Correct Envato purchase code required. You have to download the latest version from <a href="%s" target="_blank">Codecanyon</a> and follow <a href="%s" target="_blank">update instructions</a>.', 'w2dc'), 'post'), 'https://codecanyon.net/downloads', w2dc_getUpdateDocsLink());
		}
	}
