...
<script>
	var w2dc_media_metabox_attrs = <?php echo json_encode(
		array(
			...
			'images_upload_image_button' => esc_html__('Insert', 'w2dc'),
...
<div id="w2dc-images-upload-wrapper" class="w2dc-content w2dc-media-upload-wrapper <?php w2dc_esc_e($classes); ?>">
	<input type="hidden" id="w2dc-attached-images-order" name="attached_images_order" value="<?php echo implode(',', array_keys($images)); ?>">
	<h4><?php esc_html_e('Attach images', 'w2dc'); ?></h4>
	...
	<div id="w2dc-attached-images-wrapper">
		...
		<div class="w2dc-attached-item w2dc-move-label">
			<input type="hidden" name="attached_image_id[]" class="w2dc-attached-item-id" value="<?php w2dc_esc_e($attachment_id); ?>" />
			<a href="<?php w2dc_esc_e($src_full[0]); ?>" data-w2dc_lightbox="listing_images" class="w2dc-attached-item-img" style="background-image: url('<?php w2dc_esc_e($src[0]); ?>')"></a>
			...
			<div class="w2dc-attached-item-logo w2dc-radio">
				<label>
					<input type="radio" name="attached_image_as_logo" value="<?php w2dc_esc_e($attachment_id); ?>" <?php checked($logo_image, $attachment_id); ?>> <?php esc_html_e('set this image as logo', 'w2dc'); ?>
				</label>
			</div>
			<?php endif; ?>
			<div class="w2dc-attached-item-delete w2dc-fa w2dc-fa-trash-o" title="<?php esc_attr_e("delete", "w2dc"); ?>"></div>
			<div class="w2dc-attached-item-metadata"><?php w2dc_esc_e($metadata['size']); ?> (<?php w2dc_esc_e($metadata['width']); ?> x <?php w2dc_esc_e($metadata['height']); ?>)</div>
		</div>
		<?php endforeach; ?>
		<?php if (!is_admin()): ?>
		<div class="w2dc-upload-item">
			<div class="w2dc-drop-attached-item">
				<div class="w2dc-drop-zone">
					<?php esc_html_e("Drop here", "w2dc"); ?>
					<button class="w2dc-upload-item-button w2dc-btn w2dc-btn-primary"><?php esc_html_e("Browse", "w2dc"); ?></button>
...
<div id="w2dc-video-attach-wrapper" class="w2dc-content w2dc-media-upload-wrapper">
	<h4><?php esc_html_e("Attach videos", "w2dc"); ?></h4>
	
	<div id="w2dc-attached-videos-wrapper">
		<?php foreach ($videos AS $video): ?>
		<div class="w2dc-attached-item">
			<input type="hidden" name="attached_video_id[]" value="<?php w2dc_esc_e($video['id']); ?>" />
			<?php
			$image_url = '';
			...
			<div class="w2dc-attached-item-img" style="background-image: url('<?php w2dc_esc_e($image_url); ?>')"></div>
			<div class="w2dc-attached-item-delete w2dc-fa w2dc-fa-trash-o" title="<?php esc_attr_e("delete", "w2dc"); ?>"></div>
		</div>
		<?php endforeach; ?>
	</div>
	...
	<div id="w2dc-attach-videos-functions">
		<div class="w2dc-upload-option">
			<label><?php esc_html_e('Enter full YouTube or Vimeo video link', 'w2dc'); ?></label>
		</div>
		<div class="w2dc-upload-option">
