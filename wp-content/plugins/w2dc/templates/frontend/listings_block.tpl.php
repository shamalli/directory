...
?>
		...
		<div class="w2dc-content w2dc-controller" id="w2dc-controller-<?php w2dc_esc_e($frontend_controller->hash); ?>" data-controller-hash="<?php w2dc_esc_e($frontend_controller->hash); ?>" <?php if (!empty($frontend_controller->custom_home)): ?>data-custom-home="1"<?php endif; ?>>
			<style type="text/css">
				<?php if (!empty($frontend_controller->args['grid_view_logo_ratio'])): ?>
				#w2dc-controller-<?php w2dc_esc_e($frontend_controller->hash); ?> .w2dc-listings-grid .w2dc-listing-logo .w2dc-listing-logo-img-wrap:before {
					padding-top: <?php w2dc_esc_e($frontend_controller->args['grid_view_logo_ratio']); ?>%;
				}
				<?php endif; ?>
				<?php if ($frontend_controller->args['listing_thumb_width']): ?>
				#w2dc-controller-<?php w2dc_esc_e($frontend_controller->hash); ?> .w2dc-listings-block .w2dc-listing-logo .w2dc-listing-logo-img img {
					width: <?php w2dc_esc_e($frontend_controller->args['listing_thumb_width']); ?>px;
				}
				@media screen and (min-width: 769px) {
					<?php if (!is_rtl()): ?>
					#w2dc-controller-<?php w2dc_esc_e($frontend_controller->hash); ?> .w2dc-listings-block .w2dc-listing-logo-wrap {
						width: <?php w2dc_esc_e($frontend_controller->args['listing_thumb_width']); ?>px;
					...
					<?php endif; ?>
					<?php if (!is_rtl()): ?>
					#w2dc-controller-<?php w2dc_esc_e($frontend_controller->hash); ?> .w2dc-listings-block .w2dc-listing-text-content-wrap {
						<?php if (!$frontend_controller->args['wrap_logo_list_view']): ?>
						margin-left: <?php w2dc_esc_e($frontend_controller->args['listing_thumb_width']); ?>px;
						margin-right: 0;
						<?php else: ?>
						margin-left: 0;
						<?php endif; ?>
					...
					#w2dc-controller-<?php w2dc_esc_e($frontend_controller->hash); ?> .w2dc-listings-block .w2dc-listing-text-content-wrap {
						<?php if (!$frontend_controller->args['wrap_logo_list_view']): ?>
						margin-right: <?php w2dc_esc_e($frontend_controller->args['listing_thumb_width']); ?>px;
						margin-left: 0;
						<?php else: ?>
						margin-right: 0;
						<?php endif; ?>
			...
			<div class="<?php echo $frontend_controller->getListingsBlockClasses();  ?>">
				...
				<div class="w2dc-row w2dc-listings-block-header">
					...
					<div class="w2dc-found-listings">
						<?php printf(esc_html__('Found', "w2dc") . ' <span class="w2dc-badge">%d</span> %s', $frontend_controller->query->found_posts, _n($frontend_controller->getListingsDirectory()->single, $frontend_controller->getListingsDirectory()->plural, $frontend_controller->query->found_posts)); ?>
					</div>
					<?php endif; ?>
					...
					<div class="w2dc-listings-options-links">
						...
						<div class="w2dc-listings-orderby">
							<div class="w2dc-listings-orderby-dropbtn w2dc-btn w2dc-btn-primary"><?php esc_html_e('Sort by:', 'w2dc'); ?> <?php w2dc_esc_e($ordering->active_link_name); ?></div>
							<div class="w2dc-listings-orderby-links-group" role="group">
								<?php foreach ($links AS $link): ?>
								<?php if ($link['field_slug'] != $ordering->active_link || $link['order'] != $ordering->active_link_order): ?>
								<a class="w2dc-listings-orderby-link w2dc-btn" href="<?php w2dc_esc_e($link['url']); ?>" data-controller-hash="<?php w2dc_esc_e($frontend_controller->hash); ?>" data-orderby="<?php w2dc_esc_e($link['field_slug']); ?>" data-order="<?php w2dc_esc_e($link['order']); ?>" rel="nofollow">
									<?php w2dc_esc_e($link['field_name']); ?>
								</a>
								<?php endif; ?>
								<?php endforeach; ?>
							</div>
				...
				<?php endif; ?>
				<?php else: ?>
				<div class="w2dc-row w2dc-no-found-listings"><?php echo apply_filters("w2dc_no_listings_text", esc_html__("Sorry, no listings were found.", "w2dc")); ?></div>
				<?php endif; ?>
