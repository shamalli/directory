...
?>
		...
		(function($) {
			...
			$(function() {
				...
				$(document).on('mouseenter', '.w2dc-no-touch #w2dc-color-picker-panel', function () {
					$('#w2dc-color-picker-panel').stop().animate({<?php w2dc_esc_e($align); ?>: "0px"}, 500);
				});
				$(document).on('mouseleave', '.w2dc-no-touch #w2dc-color-picker-panel', function () {
					var width = $('#w2dc-color-picker-panel').width() - 50;
					$('#w2dc-color-picker-panel').stop().animate({<?php w2dc_esc_e($align); ?>: - width}, 500);
				});
	...
	<div id="w2dc-color-picker-panel" class="w2dc-content" tabindex="-1">
		<fieldset id="w2dc-color-picker">
			...
			<label><?php printf(wp_kses(__('Return to the <a href="%s">backend</a>', 'w2dc'), 'post'), admin_url('admin.php?page=w2dc_settings#_customization')); ?></label>
		</fieldset>
