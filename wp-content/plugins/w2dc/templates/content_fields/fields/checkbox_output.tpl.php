...
<div class="w2dc-field w2dc-field-output-block <?php echo $content_field->printClasses($css_classes); ?>">
	...
	<?php endif; ?>
	<ul class="w2dc-field-content w2dc-checkboxes-columns-<?php w2dc_esc_e($content_field->columns_number); ?>">
	...
	<?php foreach ($content_field->selection_items AS $key=>$item): ?>
		<li class="w2dc-field-checkbox-item-<?php echo (in_array($key, $content_field->value) ? "checked" : "unchecked"); ?>">
			...
			<span class="w2dc-field-icon w2dc-fa w2dc-fa-lg w2dc-fa-fw <?php w2dc_esc_e($content_field->icon_images[$key]); ?>"></span>
			<?php endif; ?>
			<?php w2dc_esc_e($item); ?>
		</li>
	<?php endforeach; ?>
	...
	<?php foreach ($content_field->value AS $key): ?>
		...
		<li class="w2dc-field-checkbox-item-<?php echo (in_array($key, $content_field->value) ? "checked" : "unchecked"); ?>">
			<?php if ($content_field->icon_images[$key]): ?>
			<span class="w2dc-field-icon w2dc-fa w2dc-fa-lg <?php w2dc_esc_e($content_field->icon_images[$key]); ?>"></span>
			<?php endif; ?>
			<?php w2dc_esc_e($content_field->selection_items[$key]); ?>
		</li>
		<?php endif; ?>
	<?php endforeach; ?>
	<?php endif; ?>
