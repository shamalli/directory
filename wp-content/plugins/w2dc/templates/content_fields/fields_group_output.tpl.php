...
?>
<div class="w2dc-fields-group <?php w2dc_esc_e($content_fields_group->printClasses($css_group_classes)); ?>" id="w2dc-fields-group-<?php w2dc_esc_e($content_fields_group->id); ?>">
	<?php if (!$content_fields_group->on_tab): ?>
	<div class="w2dc-fields-group-caption"><?php w2dc_esc_e($content_fields_group->name); ?></div>
	<?php endif; ?>
	...
	<?php elseif ($content_fields_group->hide_anonymous && !is_user_logged_in()): ?>
		<?php printf(wp_kses(__('You must be <a href="%s">logged in</a> to see this info', 'w2dc'), 'post'), wp_login_url(get_permalink($listing->post->ID))); ?>
	<?php endif; ?>
</div>