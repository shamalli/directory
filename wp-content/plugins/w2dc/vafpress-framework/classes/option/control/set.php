...
class W2DC_VP_Option_Control_Set
{
	...
	public function render()
	{
		// Setup data
		$data = array('set' => $this);
		return W2DC_VP_View::instance()->load('option/set', $data);
	}
	...
	public function get_fields($include_section = false)
	{
		if(!function_exists('loop_controls'))
		{
			function loop_controls($menu, $include_section)
			{
				...
				{
					if( get_class($control) === 'W2DC_VP_Option_Control_Group_Section' )
					{
						if($include_section)
						{
						...
						{
							if( W2DC_VP_Util_Reflection::field_type_from_class(get_class($field)) != 'impexp' )
							{
					...
					{
						if( W2DC_VP_Util_Reflection::field_type_from_class(get_class($control)) != 'impexp' )
						{
							$fields[$control->get_name()] = $control;
						}
					}
	...
	public function normalize_values($opt_arr)
	{
		...
		foreach ($opt_arr as $key => $value)
		{
			...
			{
				$is_multi = W2DC_VP_Util_Reflection::is_multiselectable($fields[$key]);
	...
	public function save($option_key)
	{
		...
		do_action('w2dc_vp_option_set_before_save', $opt);

		if(update_option($option_key, $opt))
		{
			$result['status']  = true;
			$result['code']    = self::SAVE_SUCCESS;
			$result['message'] = esc_html__('Saving successful', 'w2dc');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = esc_html__('Saving failed', 'w2dc');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = esc_html__('No changes made', 'w2dc');
			}
		}

		do_action('w2dc_vp_option_set_after_save', $curr_opt, $result['status'], $option_key);
	...
	public function populate_values($opt, $force_update = false)
	{
		...
		foreach ( $fields as $field )
		{
			$is_multi = W2DC_VP_Util_Reflection::is_multiselectable($field);
