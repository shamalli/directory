...
class w2dc_login_registrations {
	
	public function is_action() {
		global $w2dc_instance;
		
		$action = $w2dc_instance->action;
	...
	public function get_current_url() {
		...
		$current_url = remove_query_arg('w2dc_action', $current_url);
	...
	public function replace_url_password_message($message, $key, $user_login, $user_data) {
		...
		$url = add_query_arg('w2dc_action', 'rp', $current_url);
		$url = add_query_arg('key', $key, $url);
		$url = add_query_arg('login', rawurlencode($user_login), $url);
		
		$redirect_to = w2dc_getValue($_GET, 'redirect_to', $url);
	...
	public function process($controller = false) {
		...
		switch ($action) {
				...
				if ($http_post) {
					if (w2dc_is_recaptcha_passed()) {
						...
						if (!is_wp_error($errors)) {
							$current_url = $this->get_current_url();
							$current_url = add_query_arg('w2dc_action', 'lostpassword', $current_url);
							$current_url = add_query_arg('msg', 'checkemail', $current_url);
							
							$redirect_to = w2dc_getValue($_GET, 'redirect_to', $current_url);
			...
			case 'rp':
				...
				if (!$user || is_wp_error($user)) {
					...
					if ($user && $user->get_error_code() === 'expired_key') {
						wp_safe_redirect(add_query_arg(array('w2dc_action' => 'lostpassword', 'msg' => 'expiredkey'), $current_url));
					} else {
						wp_safe_redirect(add_query_arg(array('w2dc_action' => 'lostpassword', 'msg' => 'invalidkey'), $current_url));
					}
					exit;
				}
				...
				return array(W2DC_FSUBMIT_TEMPLATES_PATH, 'resetpassword_form.tpl.php');
			...
			case 'register':
				...
				return array(W2DC_FSUBMIT_TEMPLATES_PATH, 'registration_form.tpl.php');
			...
			default:
				...
				return array(W2DC_FSUBMIT_TEMPLATES_PATH, 'login_form.tpl.php');
		}
	}
...
function w2dc_login_form($args = array()) {
	...
	$redirect_to = w2dc_getValue($_GET, 'redirect_to', $default_redirect);
	...
	$url = add_query_arg(array('w2dc_action' => 'login', 'redirect_to' => urlencode($args['redirect'])), $current_url);
	...
	echo '
		<form name="' . esc_attr($args['form_id']) . '" id="' . esc_attr($args['form_id']) . '" action="' . esc_url($url) . '" method="post" class="w2dc_login_form" role="form">
			...
			if (get_option('w2dc_enable_recaptcha')) {
				echo '<div class="w2dc-form-group">';
				echo w2dc_recaptcha();
				echo '</div>';
			}
			echo '</div>
...
function w2dc_resetpassword_form($args = array()) {
	...
	$url = add_query_arg(array('w2dc_action' => 'resetpass', 'redirect_to' => urlencode($args['redirect'])), $current_url);
	...
	echo '
		<form name="' . esc_attr($args['form_id']) . '" id="' . esc_attr($args['form_id']) . '" action="' . esc_url($url) . '" method="post" class="w2dc_login_form" role="form">
			...
			<div class="w2dc-form-group">
				...
				<input type="password" name="pass1" id="' . esc_attr($args['id_pass1']) . '" class="w2dc-form-control" value="" />
			</div>
			<div class="w2dc-form-group">
				<label for="' . esc_attr( $args['id_pass2'] ) . '">' . esc_html($args['label_pass2']) . '</label>
				<input type="password" name="pass2" id="' . esc_attr($args['id_pass2']) . '" class="w2dc-form-control" value="" />
			</div>
			<div class="w2dc-form-group">
			...
			' . apply_filters('resetpassword_form_bottom', '', $args) . '
		</form>';

	echo '</div>';
}

function w2dc_lostpassword_form($args = array()) {
	$current_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
	$current_url = remove_query_arg('redirect_to', $current_url);
	$default_redirect = $current_url;
	...
	$url = add_query_arg(array('w2dc_action' => 'lostpassword', 'redirect_to' => urlencode($args['redirect'])), $current_url);
	...
	echo '
		<form name="' . esc_attr($args['form_id']) . '" id="' . esc_attr($args['form_id']) . '" action="' . esc_url($url) . '" method="post" class="w2dc_login_form" role="form">
			...
			<div class="w2dc-form-group">
				<label for="' . esc_attr($args['id_username']) . '">' . esc_html($args['label_username']) . '</label>
				<input type="text" name="user_login" id="' . esc_attr($args['id_username']) . '" class="w2dc-form-control" value="" />
			</div>';
	if (get_option('w2dc_enable_recaptcha')) {
		echo '<div class="w2dc-form-group">';
		echo w2dc_recaptcha();
...
function w2dc_registration_form($args = array()) {
	$current_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
	$current_url = remove_query_arg('redirect_to', $current_url);
	$default_redirect = $current_url;
	...
	$url = add_query_arg(array('w2dc_action' => 'register', 'redirect_to' => urlencode($args['redirect'])), $current_url);
	...
	echo '
		<form name="' . esc_attr($args['form_id']) . '" id="' . esc_attr($args['form_id']) . '" action="' . esc_url($url) . '" method="post" class="w2dc_login_form" role="form">
			...
			<div class="w2dc-form-group">
				...
				<input type="text" name="user_email" id="' . esc_attr($args['id_email']) . '" class="w2dc-form-control" value="" />
			</div>';
	if (get_option('w2dc_enable_recaptcha')) {
		echo '<div class="w2dc-form-group">';
		echo w2dc_recaptcha();
...
function w2dc_get_login_registration_pages() {
	...
	$pages = array();
	if (!empty($w2dc_instance->submit_pages_all)) {
		$pages = $pages + $w2dc_instance->submit_pages_all;
	}
	if (!empty($w2dc_instance->dashboard_page_id)) {
		$pages[] = array('id' => $w2dc_instance->dashboard_page_id);
	}
	$pages = apply_filters('w2dc_login_registration_pages', $pages);

	return $pages;
}

function w2dc_lostpassword_url($url) {
	$current_page_id = get_the_ID();

	$pages = w2dc_get_login_registration_pages();
...
add_filter('lostpassword_url', 'w2dc_lostpassword_url', 100);

function w2dc_register_url($url) {
	$current_page_id = get_the_ID();
	...
	foreach ($pages AS $page) {
		if ($page['id'] == $current_page_id) {
			return add_query_arg(array('w2dc_action' => 'register', 'redirect_to' => urlencode($redirect_to)), $current_page_url);
			break;
		}
	}
...
add_filter('register_url', 'w2dc_register_url', 100);

function w2dc_logout_url($url, $redirect_to) {
	$current_page_id = get_the_ID();
	...
	foreach ($pages AS $page) {
		if ($page['id'] == $current_page_id) {
			return add_query_arg(array('w2dc_action' => 'logout', 'redirect_to' => urlencode($redirect_to)), $current_page_url);
			break;
		}
	}
...
add_filter('logout_url', 'w2dc_logout_url', 100, 2);

function w2dc_login_url($url, $redirect_to) {
	$current_page_id = get_the_ID();
	...
	foreach ($pages AS $page) {
		if ($page['id'] == $current_page_id) {
			if (w2dc_get_wpml_dependent_option('w2dc_submit_login_page')) {
				return get_permalink(w2dc_get_wpml_dependent_option('w2dc_submit_login_page'));
			} else {
				return add_query_arg(array('w2dc_action' => 'login', 'redirect_to' => urlencode($redirect_to)), $current_page_url);
			}
			break;
		}
	}
...
add_filter('login_url', 'w2dc_login_url', 100, 2);
