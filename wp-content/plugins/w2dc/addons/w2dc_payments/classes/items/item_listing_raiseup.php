		...
		if (recalcPrice($listing->level->raiseup_price) > 0) {
			if (!($invoice_id = get_post_meta($listing->post->ID, '_listing_raiseup_invoice', true))) {
				$invoice_args = array(
						...
						'title' => sprintf(esc_html__('Invoice for raise up of listing: %s', 'w2dc'), $listing->title()),
				...
				if ($invoice_id = call_user_func_array('w2dc_create_invoice', $invoice_args)) {
					w2dc_addMessage(sprintf(wp_kses(__('New <a href="%s">invoice</a> was created successfully, listing will be raised up after payment', 'w2dc'), 'post'), w2dc_get_edit_invoice_link($invoice_id, 'url')));
