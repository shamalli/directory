...
<div class="w2dc-rating" <?php if ($meta_tags && $avg_rating->ratings_count): ?>itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating"<?php endif; ?>>
	...
	<?php if ($review_count = get_comments_number()): ?><meta itemprop="reviewCount" content="<?php w2dc_esc_e($review_count); ?>" /><?php endif; ?>
	<meta itemprop="ratingValue" content="<?php w2dc_esc_e($avg_rating->avg_value); ?>" />
	<meta itemprop="ratingCount" content="<?php w2dc_esc_e($avg_rating->ratings_count); ?>" />
	<?php endif; ?>
	<?php if ($show_avg): ?>
	<span class="w2dc-rating-avgvalue">
		<span><?php w2dc_esc_e($avg_rating->avg_value); ?></span>
	</span>
	<?php endif; ?>
	<div class="w2dc-rating-stars <?php if ($active): ?>w2dc-rating-active<?php endif; ?> <?php if (!empty($noajax)): ?>w2dc-rating-active-noajax<?php endif; ?>" <?php if (!empty($post_id)): ?>data-listing="<?php w2dc_esc_e($post_id); ?>"<?php endif; ?> data-nonce="<?php echo wp_create_nonce('save_rating')?>">
		...
		<?php if (!empty($noajax)): ?>
		<input type="hidden" name="w2dc-rating-noajax-<?php w2dc_esc_e($post_id); ?>" class="w2dc-rating-noajax-value" value="<?php w2dc_esc_e($avg_rating->avg_value); ?>">
		<?php endif; ?>
	</div>
</div>