...
<script>
	jQuery(document).ready(function($) {
		...
		$("#w2dc-reset-all-ratings").on('click', function() {
			if (confirm('<?php echo esc_html__('Are you sure you want to reset all ratings of this listing?', 'w2dc'); ?>')) {
				w2dc_ajax_loader_show();
				$.ajax({
					type: "POST",
					url: w2dc_js_objects.ajaxurl,
					data: {'action': 'w2dc_reset_ratings', 'post_id': <?php w2dc_esc_e($listing->post->ID); ?>},
					success: function(){
