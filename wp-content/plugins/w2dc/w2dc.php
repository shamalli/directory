...
include_once W2DC_PATH . 'classes/directories/directories_manager.php'; // remove in free version
...
include_once W2DC_PATH . 'classes/updater.php'; // remove in free version
...
class w2dc_plugin {
	...
	public function init() {
		...
		add_action('wp_loaded', array($this, 'suspend_expired_listings'));
		...
		add_filter('wpseo_primary_term_taxonomies', array($this, 'exclude_category_primary_term'));
	...
	public function loadClasses() {
		...
		$this->updater = new w2dc_updater(__FILE__, get_option('w2dc_purchase_code'));
	}
	...
	public function suspend_expired_listings() {
		...
		// suspend listings or change level
		...
		$posts_ids = $wpdb->get_col($wpdb->prepare("
				...
				WHERE
					...
					(wp_pm2.meta_value = 'active' OR wp_pm2.meta_value = 'stopped') AND
					(wp_l.eternal_active_period = '0')
			", current_time('timestamp')+(intval(get_option('w2dc_send_expiration_notification_days'))*86400)));
...
';
	...
	public function exclude_category_primary_term($taxonomies) {
		
		if (isset($taxonomies[W2DC_CATEGORIES_TAX])) {
			unset($taxonomies[W2DC_CATEGORIES_TAX]);
		}
