...
return array(
	...
	<div class="w2dc-docs w2dc-docs-side">
		...
		By default on the top appear sticky and featured listings only when they are sorted by date, in another case they are sorted exactly by title, rating, additional content fields values, but there is such option <em>"Sticky and featured listings always will be on top"</em> on <em>"Pages & Views"</em> settings tab. So it is additional tool for business model of the directory site - place listings of sticky or featured levels higher than other listings. Then at the second stage each list sorts by date (here may be useful <a href="[base_url]/documentation/directory-listings/#listings_raiseup">"raise up listings"</a> feature), except search by radius page. When user searches listings in radius - default sorting is by distance from center point.
		
		<i class="fa fa-star"></i> Some <a href="[base_url]/documentation/content-fields">content fields</a> types, like date-time, number, price, text string types have own setting to allow order by these fields. You may find special sorting settings including <em>"Default order by"</em> setting on <em>"Pages & Views"</em> settings tab.
