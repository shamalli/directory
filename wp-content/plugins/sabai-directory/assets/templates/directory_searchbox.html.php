    ...
    $('<?php echo $CURRENT_CONTAINER;?> .sabai-directory-search-radius-trigger').click(function(e){
        ...
        SABAI.popover(
            $this,
            {
                ...
                title: '<?php Sabai::_h(__('Location Options', 'sabai-directory'));?>',
                content: '<div class="sabai-directory-search-radius">'
                    + '<div class="sabai-directory-search-radius-label"><?php printf($is_mile ? Sabai::h(__('Search Radius: %s mi', 'sabai-directory')) : Sabai::h(__('Search Radius: %s km', 'sabai-directory')), '<strong></strong>');?></div>'
                    + '<div class="sabai-directory-search-radius-slider" style="margin-top:5px;"></div>'
                    + '</div>'
                    + '<button style="display:none; margin-top:20px !important; width:auto;" class="sabai-btn sabai-btn-xs sabai-btn-default sabai-directory-search-geolocate"><i class="fa fa-map-marker"></i> <?php Sabai::_h(__('Get My Location', 'sabai-directory'));?></button>'
            }
        );
            ...
            geocoder.geocode({address: location.val()<?php if ($search['country']):?>, region: '<?php echo $search['country'];?>'<?php endif;?>}, function(results, status) {
                switch (status) {
                    ...
                    case google.maps.GeocoderStatus.ZERO_RESULTS:
                        alert('<?php Sabai::_h(__('Invalid location', 'sabai-directory'));?>');
                        break;
                    default:
                        alert(status);
                }
