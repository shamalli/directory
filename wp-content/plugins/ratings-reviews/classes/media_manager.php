...
class w2rr_media_manager {
	...
	public function __construct() {
		add_action('wp_ajax_w2rr_upload_image', array($this, 'upload_image'));
		add_action('wp_ajax_nopriv_w2rr_upload_image', array($this, 'upload_image'));

		add_action('wp_ajax_w2rr_upload_media_image', array($this, 'upload_media_image'));
		
		add_action('wp_ajax_w2rr_remove_image', array($this, 'remove_image'));
		add_action('wp_ajax_nopriv_w2rr_remove_image', array($this, 'remove_image'));
		...
		if (get_option('w2rr_prevent_users_see_other_media')) {
	...
	public function mediaMetabox($post = null, $args = array()) {
		w2rr_renderTemplate('media_metabox.tpl.php' , array_merge(array(
				'images' => $this->images,
				'videos' => $this->videos,
				'logo_image' => $this->logo_image,
				'classes' => (!empty($args['args']['target']) ? 'w2rr-media-metabox-'.$args['args']['target'] : '')
			), $this->params
		));
	}
	...
	public function upload_image() {
		...
		if (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file']) && ($_FILES['browse_file']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if ($post = get_post($post_id)) {
				if (current_user_can('edit_post', $post_id) || !$post->post->post_author) {
					$do_upload = apply_filters('w2rr_count_attachments', true, $post_id);
					if ($do_upload) {
	...
	public function admin_enqueue_scripts_styles() {
		...
		if (is_admin() && current_user_can('upload_files')) {
			...
			wp_register_script('w2rr_fileupload', W2RR_RESOURCES_URL . 'js/jquery.fileupload.js', array('jquery'), false, true);
			wp_register_script('w2rr_fileupload_iframe', W2RR_RESOURCES_URL . 'js/jquery.iframe-transport.js', array('jquery'), false, true);
			wp_enqueue_script('w2rr_fileupload');
			wp_enqueue_script('w2rr_fileupload_iframe');
		}
	}
}
