...
class w2rr_comments_manager {
	
	public function __construct() {
		add_action('wp_ajax_w2rr_comments_add_comment', array($this, 'comments_add_comment'));
	    add_action('wp_ajax_nopriv_w2rr_comments_add_comment', array($this, 'comments_add_comment'));
	    add_action('wp_ajax_w2rr_comments_load_template', array($this, 'comments_load_template'));
	    add_action('wp_ajax_nopriv_w2rr_comments_load_template', array($this, 'comments_load_template'));
	    
	    add_action('add_meta_boxes', array($this, 'remove_comments_metabox'), 302);
	}
	
	/**
	 * remove comments metaboxes on W2RR Reviews
	 *
	 */
	public function remove_comments_metabox() {
		remove_meta_box('commentstatusdiv', W2RR_REVIEW_TYPE, 'normal');
	}

	public function comments_load_template() {
		check_ajax_referer('w2rr_comments_nonce', 'security');
	...
	public function display_comments_node($post_id, $parent = 0) {
		...
		if ($comments):
		?>
		<div class="w2rr-comments-container">
	...
	public function display_comment($comment) {
		...
		?>
		<div class="w2rr-comments-content w2rr-comments-<?php echo esc_attr($class); ?>" id="comment-<?php echo esc_attr($comment->comment_ID); ?>">
			<div class="w2rr-comments-p">
				<?php w2rr_comments_profile_pic($comment->comment_author_email); ?>
...
function w2rr_comments_profile_pic($id_or_email = null, $email = null) {
	...
	echo '<span class="w2rr-comments-profile-pic-container">' . $html . '</span>';
}
