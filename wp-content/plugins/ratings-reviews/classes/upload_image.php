...
add_action('wp_ajax_w2rr_upload_image_single', 'w2rr_upload_image_single');
add_action('wp_ajax_nopriv_w2rr_upload_image_single', 'w2rr_upload_image_single');
function w2rr_upload_image_single() {
	$name = w2rr_getValue($_GET, 'input_name');
	$upload = new w2rr_upload_image(esc_attr($name));
	$upload->do_upload();
}

class w2rr_upload_image {
	public $input_name;
	public $action_url;
	...
	public function get_field_name() {
		return 'w2rr-upload-image-input-' . $this->input_name;
	}
	
	public function get_attachment_id_from_post() {
		$field = $this->get_field_name();
		return w2rr_getValue($_POST, $field, '');
	}
	
	public function display_form() {
		$this->action_url = admin_url('admin-ajax.php?action=w2rr_upload_image_single&input_name='.$this->input_name.'&_wpnonce='.wp_create_nonce('upload_images'));
		
		w2rr_renderTemplate(W2RR_TEMPLATES_PATH . 'upload_image.tpl.php', array('upload' => $this));
	}
