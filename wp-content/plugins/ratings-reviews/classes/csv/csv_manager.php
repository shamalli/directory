...
class w2rr_csv_manager {
	...
	public function __construct() {
		...
		// Export
		if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'w2rr_csv_import' && isset($_REQUEST['action']) && $_REQUEST['action'] == 'export_settings' && isset($_REQUEST['csv_export'])) {
			add_action('admin_init', array($this, 'csvExport'));
		}
		// Export Images
		if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'w2rr_csv_import' && isset($_REQUEST['action']) && $_REQUEST['action'] == 'export_settings' && isset($_REQUEST['export_images'])) {
	...
	public function createHelper() {
		if (!$this->import_type) {
			$this->import_type = w2rr_getValue($_POST, 'import_type');
		}
		if ($this->import_type == 'create_reviews' || $this->import_type == 'update_reviews') {
			$this->import_export_helper = new w2rr_csv_import_export_reviews($this);
		}
		
		if (!$this->export_type) {
			$this->export_type = w2rr_getValue($_POST, 'export_type');
		}
		if ($this->export_type == 'export_reviews') {
			$this->import_export_helper = new w2rr_csv_import_export_reviews($this);
		}
	}
	...
	public function w2rr_csv_import() {
	...
	public function csvImportSettings($vars = array()) {
		...
		w2rr_renderTemplate('csv_manager/import_settings.tpl.php', $vars);
	}
	...
	public function csvCollateColumns() {
		...
		if ((w2rr_getValue($_POST, 'submit') || w2rr_getValue($_POST, 'goback')) && wp_verify_nonce($_POST['w2rr_csv_import_nonce'], W2RR_PATH) && (!defined('W2RR_DEMO') || !W2RR_DEMO)) {
			$errors = false;
			...
			if ($validation->run()) {
				...
				w2rr_renderTemplate('csv_manager/collate_columns.tpl.php', $template_fields);
			} else {
				w2rr_addMessage($validation->error_array(), 'error');
	...
	public function csvImport() {
		if ((w2rr_getValue($_POST, 'submit') || w2rr_getValue($_POST, 'tsubmit')) && wp_verify_nonce($_POST['w2rr_csv_import_nonce'], W2RR_PATH) && (!defined('W2RR_DEMO') || !W2RR_DEMO)) {
			if (w2rr_getValue($_POST, 'tsubmit'))
				$this->test_mode = true;
			...
			if ($validation->run()) {
				...
				w2rr_renderTemplate('csv_manager/import_results.tpl.php', $template_fields);
			} else {
				w2rr_addMessage($validation->error_array(), 'error');
	...
	public function extractImages($zip_file) {
		$dir = trailingslashit(get_temp_dir() . 'w2rr_' . time());
	...
	public function processImages($post_id, &$item_data, $line_n) {
		...
		foreach ($item_data['images'] AS $image_item) {
			...
			if ($this->images_dir) {
				...
				$subdir = w2rr_getValue(wp_upload_dir(null, false, true), 'subdir', '');
	...
	public function exportImages() {
		...
		$images = apply_filters("w2rr_csv_export_images", $images);

		if ($images) {
			$zip_file = trailingslashit(get_temp_dir()) . 'w2rr_images.zip';
			...
			header("Cache-Control: private", false);
			header("Content-Type: application/octet-stream");
			header('Content-Disposition: attachment; filename="w2rr_images.zip"');
