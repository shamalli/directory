...
class W2RR_VP_Metabox_Depsloader
{

	/**
	 * ARRAY OF W2RR_VP_METABOX_ALCHEMY OBJECT
	 * @var [type]
	 */
	private $things;
	...
	{
		$w2rr_vp_metabox_used = false;
		$metaboxes = $this->things;

		$result = array(
			'scripts'              => array(),
			'styles'               => array(),
			'localize_name'        => 'w2rr_vp_mb',
		...
		$script_always = W2RR_VP_Util_Config::instance()->load('dependencies', 'scripts.always');
		$style_always  = W2RR_VP_Util_Config::instance()->load('dependencies', 'styles.always');
		...
		if(is_array($metaboxes)) foreach ($metaboxes as $key => $metabox)
		{
			if($metabox->can_output())
			{
				...
				$w2rr_vp_metabox_used = true;
			}

			if($w2rr_vp_metabox_used)
			{
