...
class VP_W2RR_Metabox_Depsloader
{

	/**
	 * ARRAY OF VP_W2RR_METABOX_ALCHEMY OBJECT
	 * @var [type]
	 */
	private $things;
	...
	{
		$vp_w2rr_metabox_used = false;
		$metaboxes = $this->things;

		$result = array(
			'scripts'              => array(),
			'styles'               => array(),
			'localize_name'        => 'vp_w2rr_mb',
		...
		$script_always = VP_W2RR_Util_Config::instance()->load('dependencies', 'scripts.always');
		$style_always  = VP_W2RR_Util_Config::instance()->load('dependencies', 'styles.always');
		...
		if(is_array($metaboxes)) foreach ($metaboxes as $key => $metabox)
		{
			if($metabox->can_output())
			{
				...
				$vp_w2rr_metabox_used = true;
			}

			if($vp_w2rr_metabox_used)
			{
