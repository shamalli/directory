...
class W2RR_VP_ShortcodeGenerator
{
	...
	{
		$this->main_image     = W2RR_VP_PUBLIC_URL . '/img/w2rr_vp_shortcode_icon.png';
		$this->sprite_image   = W2RR_VP_PUBLIC_URL . '/img/w2rr_vp_shortcode_icon_sprite.png';
	...
	{
		$post_type  = W2RR_VP_Metabox::_get_current_post_type();
	...
	public function can_output()
	{
		...
		// if in post / page
		if( W2RR_VP_Metabox::_is_post_or_page() )
		{
			// then consider the types
			if( !in_array("*", $this->types) ) // if wildcard exists, then always shows
				$can &= in_array(W2RR_VP_Metabox::_get_current_post_type(), $this->types);
			else
				$can &= true;
		}
		else
	...
	public function print_modal()
	{
		...
		?>
		<div id="<?php w2rr_esc_e($modal_id); ?>" class="vp-sc-dialog reveal-modal xlarge">
			<h1><?php w2rr_esc_e($this->modal_title); ?></h1>
			<div class="vp-sc-scroll-container">
				<div class="vp-sc-wrapper">
					...
					<?php foreach ($this->template as $title => $menu): ?>
						...
						<li class="current"><a href="#<?php echo str_replace(' ', '_', $title); ?>"><?php w2rr_esc_e($title); ?></li></a>
						<?php else: ?>
						<li><a href="#<?php echo str_replace(' ', '_', $title); ?>"><?php w2rr_esc_e($title); ?></li></a>
						<?php endif; ?>
					<?php endforeach; ?>
					</ul>
							...
							<?php foreach ($menu['elements'] as $name => $element): ?>
								<li class="vp-sc-element postbox<?php if(isset($element['attributes'])) echo ' has-options'; ?><?php if(isset($element['active']) && $element['active'] == true) echo ' active'; ?>">
									...
									<form class="vp-sc-element-form <?php if(!isset($element['active']) || isset($element['active']) && $element['active'] == false):?>vp-hide<?php endif; ?> inside">
										<?php w2rr_esc_e($this->print_form($element['attributes'])); ?>
									</form>
									<?php endif; ?>
								</li>
							<?php endforeach; ?>
	...
	function print_form($attributes)
	{
		...
		{
			// create the object
			$make           = W2RR_VP_Util_Reflection::field_class_from_type($attr['type']);
			// prefix name
			...
			<?php if($attr['type'] !== 'notebox'): ?>
				<div class="vp-sc-field vp-<?php w2rr_esc_e($attr['type']); ?>" data-vp-type="vp-<?php w2rr_esc_e($attr['type']); ?>">
					<div class="label"><label><?php w2rr_esc_e($attr['label']); ?></label></div>
					<div class="field"><div class="input"><?php echo $field->render(true); ?></div></div>
				</div>
			<?php else: ?>
				<?php $status = isset($attr['status']) ? $attr['status'] : 'normal'; ?>
				<div class="vp-sc-field vp-<?php w2rr_esc_e($attr['type']); ?> note-<?php w2rr_esc_e($status); ?>" data-vp-type="vp-<?php w2rr_esc_e($attr['type']); ?>">
		...
		<div class="vp-sc-action">
			<button class="vp-sc-insert button"><?php esc_html_e('Insert', 'w2rr'); ?></button>
			<button class="vp-sc-cancel button"><?php esc_html_e('Cancel', 'w2rr') ?></button>
		</div>
		<?php
	}
	...
	{
		if( W2RR_VP_Metabox::_is_post_or_page() && !current_user_can( 'edit_posts' ) &&
	...
	public static function print_styles($buttons)
	{
		?>
			<style type="text/css">
				...
				#qt_content_<?php w2rr_esc_e($sg->name); ?>{
					background: url('<?php w2rr_esc_e($sg->sprite_image); ?>') 2px -21px no-repeat !important;
					text-indent: -999px;
				}
				span.mce_<?php w2rr_esc_e($sg->name); ?>{
					background: url('<?php w2rr_esc_e($sg->sprite_image); ?>') 0 0 no-repeat !important;
				}
				<?php endforeach; ?>
			</style>
		<?php
	...
	public static function register_buttons($buttons)
	{
		...
		{
			if( $sg->can_output() )
				$w2rr_vp_buttons[] = $sg->name;
		}
		$buttons = array_merge($buttons, $w2rr_vp_buttons);
		return $buttons;
	}

	public static function add_buttons($plugin_array)
	{
		$plugin_array['w2rr_vp_sc_button'] = W2RR_VP_PUBLIC_URL .'/js/shortcodes.js';
