 ...
 */
abstract class W2RR_VP_Control_FieldMulti extends W2RR_VP_Control_Field
{
	 ...
	 * @return W2RR_VP_Control_FieldMulti Field object
	 */
	protected function _basic_make($arr)
	{
		...
		if (!empty($arr['items']))
		{
			...
			{
				$the_item = new W2RR_VP_Control_Field_Item_Generic();
		...
		if (isset($arr['default']))
		{
			...
			if(!W2RR_VP_Util_Reflection::is_multiselectable($this))
			{
		...
		foreach ($this->_raw_default as $def)
		{
			switch ($def)
			{
				case '{{all}}':
					...
					break;
				case '{{first}}':
					$first = W2RR_VP_Util_Array::first($items);
					if(!is_null($first))
	...
	public function set_default($_default) {
		if(is_array($_default) and !W2RR_VP_Util_Reflection::is_multiselectable($this))
			$_default = W2RR_VP_Util_Array::first($_default);
	...
	/**
	 * Add single item
	 * @param W2RR_VP_Control_Field_Item_ $opt Single item item
	 */
