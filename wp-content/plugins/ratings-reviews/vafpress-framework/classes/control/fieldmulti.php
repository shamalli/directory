 ...
 */
abstract class VP_W2RR_Control_FieldMulti extends VP_W2RR_Control_Field
{
	 ...
	 * @return VP_W2RR_Control_FieldMulti Field object
	 */
	protected function _basic_make($arr)
	{
		...
		if (!empty($arr['items']))
		{
			...
			{
				$the_item = new VP_W2RR_Control_Field_Item_Generic();
		...
		if (isset($arr['default']))
		{
			...
			if(!VP_W2RR_Util_Reflection::is_multiselectable($this))
			{
		...
		foreach ($this->_raw_default as $def)
		{
			switch ($def)
			{
				case '{{all}}':
					...
					break;
				case '{{first}}':
					$first = VP_W2RR_Util_Array::first($items);
					if(!is_null($first))
	...
	public function set_default($_default) {
		if(is_array($_default) and !VP_W2RR_Util_Reflection::is_multiselectable($this))
			$_default = VP_W2RR_Util_Array::first($_default);
	...
	/**
	 * Add single item
	 * @param VP_W2RR_Control_Field_Item_ $opt Single item item
	 */
