	...
	$(document).ready(function(){
		...
		jQuery.fn.scReset = function(){
			...
			if ($.fn.select2vp)
			{
				...
				w2rr_vp.init_fontawesome_chooser($(this).find('.vp-js-fontawesome'));
				w2rr_vp.init_select2vp($(this).find('.vp-js-select2vp'));
				w2rr_vp.init_sorter($(this).find('.vp-js-sorter'));
			}
			return $(this);
		};

		// init controls
		w2rr_vp.init_controls($('.vp-sc-main'));

		// shortcode image controls event bind
		w2rr_vp.custom_check_radio_event(".vp-sc-dialog", ".vp-checkimage .field .input label");
		w2rr_vp.custom_check_radio_event(".vp-sc-dialog", ".vp-radioimage .field .input label");
		...
		$('.vp-sc-element .vp-sc-element-heading a').on('click.w2rr_vp_sc', function(e){
			e.preventDefault();
			...
			else
			{
				...
				$modal.find('.vp-sc-element-form').w2rr_vp_slideUp();

				if($form.exists())
				{
					$parent.addClass('active');
					$form.w2rr_vp_slideDown();
				}
				else
				{
					code = decodeEntities(code);
					$modal.trigger('w2rr_vp_insert_shortcode', code);
					$modal.trigger('reveal:close');
				}
			}
		});

		$('.vp-sc-insert').on('click.w2rr_vp_sc_insert', function(e){
			e.preventDefault();
			var $modal  = $(this).parents('.vp-sc-dialog'),
			...
			$form.on('submit', function(e){w2rr_vp.tinyMCE_save(); e.preventDefault();});
			$form.trigger('submit');
		...
		$('.vp-sc-cancel').on('click.w2rr_vp_sc_cancel', function(e){
			e.preventDefault();
			$('.vp-sc-element').removeClass('active');
