	...
	$(document).ready(function(){
		...
		jQuery.fn.scReset = function(){
			...
			if ($.fn.select2vp)
			{
				...
				vp_w2rr.init_fontawesome_chooser($(this).find('.vp-js-fontawesome'));
				vp_w2rr.init_select2vp($(this).find('.vp-js-select2vp'));
				vp_w2rr.init_sorter($(this).find('.vp-js-sorter'));
			}
			return $(this);
		};

		// init controls
		vp_w2rr.init_controls($('.vp-sc-main'));

		// shortcode image controls event bind
		vp_w2rr.custom_check_radio_event(".vp-sc-dialog", ".vp-checkimage .field .input label");
		vp_w2rr.custom_check_radio_event(".vp-sc-dialog", ".vp-radioimage .field .input label");
		...
		$('.vp-sc-element .vp-sc-element-heading a').bind('click.vp_w2rr_sc', function(e){
			e.preventDefault();
			...
			else
			{
				...
				$modal.find('.vp-sc-element-form').vp_w2rr_slideUp();

				if($form.exists())
				{
					$parent.addClass('active');
					$form.vp_w2rr_slideDown();
				}
				else
				{
					code = decodeEntities(code);
					$modal.trigger('vp_w2rr_insert_shortcode', code);
					$modal.trigger('reveal:close');
				}
			}
		});

		$('.vp-sc-insert').bind('click.vp_w2rr_sc_insert', function(e){
			e.preventDefault();
			var $modal  = $(this).parents('.vp-sc-dialog'),
			...
			$form.on('submit', function(e){vp_w2rr.tinyMCE_save(); e.preventDefault();});
			$form.submit();
		...
		$('.vp-sc-cancel').bind('click.vp_w2rr_sc_cancel', function(e){
			e.preventDefault();
			$('.vp-sc-element').removeClass('active');
