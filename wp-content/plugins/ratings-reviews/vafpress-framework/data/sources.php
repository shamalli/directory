...
function w2rr_vp_get_categories()
{
...
function w2rr_vp_get_users()
{
	$wp_users = W2RR_VP_WP_User::get_users();
...
function w2rr_vp_get_roles()
{
	$result         = array();
	$editable_roles = W2RR_VP_WP_User::get_editable_roles();
...
function w2rr_vp_get_gwf_family()
{
...
W2RR_VP_Security::instance()->whitelist_function('w2rr_vp_get_gwf_weight');

function w2rr_vp_get_gwf_weight($face)
{
	if(empty($face))
		return array();
...
W2RR_VP_Security::instance()->whitelist_function('w2rr_vp_get_gwf_style');

function w2rr_vp_get_gwf_style($face)
{
	if(empty($face))
		return array();
...
W2RR_VP_Security::instance()->whitelist_function('w2rr_vp_get_gwf_subset');

function w2rr_vp_get_gwf_subset($face)
{
	if(empty($face))
		return array();
...
function w2rr_vp_get_social_medias() {
	$socmeds = array(
...
function w2rr_vp_get_fontawesome_icons()
{
	// scrape list of icons from fontawesome css
	if( false == ( $icons  = get_transient( 'w2rr_vp_fontawesome_icons' ) ) )
	{
		$pattern = '/\.(w2rr-fa-(?:\w+(?:-)?)+):before\s*{\s*content/';
		$subject = file_get_contents(W2RR_VP_DIR . '/public/css/vendor/font-awesome.min.css');
		...
		set_transient( 'w2rr_vp_fontawesome_icons', $icons, 60 * 60 * 24 );
	}

	return $icons;
}

W2RR_VP_Security::instance()->whitelist_function('w2rr_vp_dep_boolean');

function w2rr_vp_dep_boolean($value)
{
	$args   = func_get_args();
	$result = true;
