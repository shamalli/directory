...
function vp_w2rr_get_categories()
{
...
function vp_w2rr_get_users()
{
	$wp_users = VP_W2RR_WP_User::get_users();
...
function vp_w2rr_get_roles()
{
	$result         = array();
	$editable_roles = VP_W2RR_WP_User::get_editable_roles();
...
function vp_w2rr_get_gwf_family()
{
...
VP_W2RR_Security::instance()->whitelist_function('vp_w2rr_get_gwf_weight');

function vp_w2rr_get_gwf_weight($face)
{
	if(empty($face))
		return array();
...
VP_W2RR_Security::instance()->whitelist_function('vp_w2rr_get_gwf_style');

function vp_w2rr_get_gwf_style($face)
{
	if(empty($face))
		return array();
...
VP_W2RR_Security::instance()->whitelist_function('vp_w2rr_get_gwf_subset');

function vp_w2rr_get_gwf_subset($face)
{
	if(empty($face))
		return array();
...
function vp_w2rr_get_social_medias() {
	$socmeds = array(
...
function vp_w2rr_get_fontawesome_icons()
{
	// scrape list of icons from fontawesome css
	if( false == ( $icons  = get_transient( 'vp_w2rr_fontawesome_icons' ) ) )
	{
		$pattern = '/\.(w2rr-fa-(?:\w+(?:-)?)+):before\s*{\s*content/';
		$subject = file_get_contents(VP_W2RR_DIR . '/public/css/vendor/font-awesome.min.css');
		...
		set_transient( 'vp_w2rr_fontawesome_icons', $icons, 60 * 60 * 24 );
	}

	return $icons;
}

VP_W2RR_Security::instance()->whitelist_function('vp_w2rr_dep_boolean');

function vp_w2rr_dep_boolean($value)
{
	$args   = func_get_args();
	$result = true;
