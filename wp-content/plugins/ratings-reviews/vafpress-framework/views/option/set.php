<div class="wrap">
	...
	<div id="vp-wrap" class="vp-wrap">
		<div id="vp-option-panel"class="vp-option-panel <?php echo ($set->get_layout() === 'fixed') ? 'fixed-layout' : 'fluid-layout' ; ?>">
			<div class="vp-left-panel">
				<div id="vp-logo" class="vp-logo">
					<img src="<?php echo VP_W2RR_Util_Res::img($set->get_logo()); ?>" alt="<?php echo $set->get_title(); ?>" />
				</div>
					...
					<ul class="vp-menu-level-1">
						...
						<?php endif; ?>
							...
							<?php endif; ?>
								<?php
								$icon = $menu->get_icon();
								$font_awesome = VP_W2RR_Util_Res::is_font_awesome($icon);
								if ($font_awesome !== false):
									VP_W2RR_Util_Text::print_if_exists($font_awesome, '<i class="w2rr-fa %s"></i>');
								else:
									VP_W2RR_Util_Text::print_if_exists(VP_W2RR_Util_Res::img($icon), '<i class="custom-menu-icon" style="background-image: url(\'%s\');"></i>');
								endif;
								?>
							...
							<ul class="vp-menu-level-2">
								...
								<?php endif; ?>
									<a href="#<?php echo $submenu->get_name(); ?>" class="vp-js-menu-goto vp-menu-goto">
										...
										$font_awesome = VP_W2RR_Util_Res::is_font_awesome($sub_icon);
										if ($font_awesome !== false):
											VP_W2RR_Util_Text::print_if_exists($font_awesome, '<i class="w2rr-fa %s"></i>');
										else:
											VP_W2RR_Util_Text::print_if_exists(VP_W2RR_Util_Res::img($sub_icon), '<i class="custom-menu-icon" style="background-image: url(\'%s\');"></i>');
										endif;
										?>
			...
			<div class="vp-right-panel">
				<form id="vp-option-form" class="vp-option-form vp-js-option-form" method="POST">
					<div id="vp-submit-top" class="vp-submit top">
						<div class="inner">
							<input class="vp-save vp-button button button-primary" type="submit" value="<?php esc_attr_e('Save Changes', 'vp_w2rr_textdomain'); ?>" />
							<p class="vp-js-save-loader save-loader" style="display: none;"><img src="<?php VP_W2RR_Util_Res::img_out('ajax-loader.gif', ''); ?>" /><?php _e('Saving Now', 'vp_w2rr_textdomain'); ?></p>
		...
		<div id="vp-copyright" class="vp-copyright">
			<p><?php printf(__('This option panel is built using Vafpress Framework %s powered by Vafpress', 'vp_w2rr_textdomain'), VP_W2RR_VERSION); ?></p>
		</div>
	</div>
