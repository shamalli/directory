...
class WPAlchemy_MetaBox
{
	...
	static function _get_post_id()
	{
		...
		$p_post_id = isset($_POST['post_ID']) ? sanitize_text_field($_POST['post_ID']) : null ;

		$g_post_id = isset($_GET['post']) ? sanitize_text_field($_GET['post']) : null ;

		$post_id = $g_post_id ? $g_post_id : $p_post_id ;

		// this line commented not to mess up post types,
		// for some reason some plugin inject into the global $post
	...
	function can_output()
	{
		...
		(
			...
			!empty($this->exclude_tags) OR 
			!empty($this->include_tag) OR
			!empty($this->include_tags)
		)
		{
		...
		)
		{
			...
			if (!empty($this->exclude_tags))
			{
				foreach ($tags as $tag)
				{
					if (in_array($tag->term_id,$this->exclude_tags)) 
					{
						$can_output = FALSE;
						break;
					}
			...
			if (!empty($this->include_tags))
			{
				foreach ($tags as $tag)
				{
					if (in_array($tag->term_id,$this->include_tags)) 
					{
						$can_output = TRUE;
						break;
					}
